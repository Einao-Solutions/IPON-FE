<script lang="ts">
  ...
  import TradePubsTable from "../home/trademarkpubs/TradePubsTable.svelte";
...
<div class="w-full bg-white">
  <div class="max-w-6xl mx-auto px-4 py-8 sm:px-6">
    <!-- Header -->
    <div class="border-b-4 border-green-700 pb-6 mb-8 flex items-center gap-4">
      <img src={cldLogo} alt="CLD Logo" class="h-12" />
      <div>
    ...
    <div class="flex flex-col sm:flex-row gap-4 items-end mb-8">
      <div class="flex-1 flex flex-col gap-2">
        <label class="text-xs font-bold text-black uppercase tracking-wider"
          >Search</label
        >
        <Input
    ...
    {#if isLoading}
      <div
        class="flex items-center justify-center min-h-96 bg-gray-50 border border-gray-300 rounded-none"
      >
    ...
    {:else if dataList.length > 0}
      <div
        class="bg-gray-50 border-l-4 border-green-700 border border-gray-300 rounded-none p-4 mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3"
      >
        <span class="text-sm text-gray-800 font-medium">
          Found <strong class="text-green-700">{count}</strong> result{count !==
          1
            ? "s"
            : ""}
        </span>
        <span class="text-sm text-gray-800 font-medium">
          Showing <strong class="text-green-700"
            >{Math.min(dataList.length, 10)}</strong
          >
          of
      ...
      <div class="border border-gray-300 rounded-none overflow-hidden">
        <TradePubsTable
          ...
          {count}
          {searchTitle}
          on:pageChange={(e) => carryOutSearch(e.detail.index, false)}
        />
      </div>
    {:else}
