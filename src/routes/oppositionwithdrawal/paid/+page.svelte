<script lang="ts">
	...
	onMount(async () => {
		...
		await updateWithdrawalPayment(paymentId);
	});

	async function updateWithdrawalPayment(paymentId: string | null) {
		if (!paymentId) {
		...
		const result = await fetch(
			`${baseURL}/api/opposition/UpdateOppositionWithdrawalPayment?paymentId=${paymentId}`,
			{
				method: 'POST',
		...
		if (result.ok) {
			...
			sessionStorage.removeItem('oppositionWithdrawalPayload');
		} else {
...
<main class="flex justify-center items-center min-h-screen bg-gray-50">
	<div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
		...
		<h1 class="text-3xl font-bold text-gray-900 mb-8 text-center">Opposition Withdrawal</h1>
		...
		{#if subMessageVisible}
			...
			>
				YOUR WITHDRAWAL REQUEST HAS BEEN RECEIVED AND IS RECEIVING DUE ATTENTION
			</p>
			...
			<div class="mt-4 flex flex-col gap-3" in:fade={{ duration: 300, delay: 300 }}>
				<a href="https://www.iponigeria.com" target="_blank" rel="noopener noreferrer">
					<button
						class="w-full bg-orange-600 hover:bg-orange-700 text-white font-semibold py-3 px-6 rounded-md transition-colors duration-200"
					>
