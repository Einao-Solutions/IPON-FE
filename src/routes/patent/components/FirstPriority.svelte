<script lang="ts">
  ...
  let localPriorityInfo = get(patentForm).firstPriorityInfo.length
    ? [...get(patentForm).firstPriorityInfo]
    : [
        {
          number: '',
          Country: '',
  ...
  onMount(() => {
    ...
    // but prefill nothing here; we will populate per-row on focus.
  });

  function updateStore() {
    patentForm.update(form => {
      form.firstPriorityInfo = [...localPriorityInfo];
      return form;
    });
  }
...
<div class="space-y-6">
  <h2 class="text-2xl font-semibold">First Priority Information</h2>

  {#each localPriorityInfo as info, index}
    <div class="border p-4 rounded-md mb-4 space-y-4">
      <h3 class="text-sm font-medium mb-2">First Priority Information {index + 1}</h3>
  ...
  <!-- <button class="btn-black" on:click={addPriorityInfo}>+ Add First Priority Info</button> -->
