<script lang="ts">
  ...
  async function saveNewStatus() {
    ...
    const userName =
      ...
      [user?.firstName?.trim(), user?.lastName?.trim()]
        ...
        .join(" ");
    if (!user?.id || !userName) {
      toast.error("Your user details are incomplete. Please sign in again.", {
        position: "top-right",
      });
      return;
    }
    if (!selectedRevision || selectedStatus === null || !$applicationData) {
      toast.error("Select a revision and status before continuing.", {
        position: "top-right",
      });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          validationErrors ||
            problem?.title ||
            `Unable to update status (${res.status}).`,
        );
      }
      if (res.ok) {
        const latestData = await res.json();
        const curr = (latestData.applicationHistory as any[]).find(
          (x: any) => x.id === selectedRevision?.id,
        );
  ...
  async function ProceedToNext() {
    if (selectedRevision?.status === ApplicationStatuses.AwaitingConfirmation) {
      ...
      selectedRevision?.status === ApplicationStatuses.AwaitingPayment
    ) {
      // proceed to payment
      await goto(
  ...
  async function saveApplication() {
    const upperChar =
      ...
      ((selectedRevision?.field?.length ?? 0) > 1
        ? selectedRevision?.field?.slice(1)
        : "");
...
<div>
  <AlertDialog.Root bind:open={showConfirmUserDateUpdate}>
    <AlertDialog.Content>
      <AlertDialog.Header>
        <AlertDialog.Title
          >{selectedRevision?.field} Change confirmation</AlertDialog.Title
        >
      ...
      <AlertDialog.Footer>
        ...
        <AlertDialog.Action on:click={() => updateField()}
  ...
  <Dialog.Root bind:open={showUpdateDialog} onOpenChange={resetForm}>
    <Dialog.Content class="max-w-fit">
      ...
      <Dialog.Footer class="sm:flex gap-3">
        ...
        >
        <Button on:click={() => (showUpdateConfirmation = true)}
          >Continue</Button
        >
      </Dialog.Footer>
    </Dialog.Content>
      ...
      <Table.Body>
        {#each $revisions as revision, i (i)}
          <Table.Row>
            ...
            <Table.Cell
              ><AppStatusTag value={revision.status ?? undefined} /></Table.Cell
            >
