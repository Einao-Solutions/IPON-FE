<script lang="ts">
	...
	function loadData() {
		...
		if (type === 1 || type===null) {
			...
			const isStaffOrAdmin: boolean = user?.roles.includes(UserRoles.Support) || user?.roles.includes(UserRoles.BackOffice)
			...
			const rev_data={
				...
				isUserOrAdmin: [UserRoles.Support, UserRoles.Agent].some(x=>$loggedInUser.roles.includes(x))
			}
