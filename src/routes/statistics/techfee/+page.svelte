<script lang="ts">
  ...
  let selectedStartDate: DateValue | undefined = today(getLocalTimeZone()).set({
    day: 1,
  });
  let selectedEndDate: DateValue | undefined = today(getLocalTimeZone());
  ...
  function renderCharts() {
    ...
    const paymentLabels = firstPeriod.paymentTypes.map(
        ...
        datasets: results.periods.map((period, index) => ({
          ...
          data: paymentLabels.map((label) => {
            const found = period.paymentTypes.find(
              (pt) => (pt.paymentType || "Unknown") === label,
            );
    ...
    const labels = firstPeriod.paymentTypes.map(
      (pt) => pt.paymentType || "Unknown",
    );
  ...
  function validatePeriodSelection(): boolean {
    ...
    if (selectedStartDate.compare(selectedEndDate) > 0) {
  ...
  async function fetchSingle() {
    ...
    try {
      ...
      const response = await fetch(
        `${baseURL}/api/statistics/finance/techfee/compare`,
        {
          method: "POST",
      ...
      if (!response.ok)
        throw new Error(data.error || "Failed to fetch tech fee statistics");
      results = data.data;
    } catch (e) {
  ...
  function handleClearAll() {
    ...
    selectedStartDate = today(getLocalTimeZone()).set({ day: 1 });
    selectedEndDate = today(getLocalTimeZone());
  }
    ...
    <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6 mb-6">
      ...
      >
        <div class="flex items-start gap-4 flex-1">
          <div
            class="flex-shrink-0 w-14 h-14 bg-blue-600 rounded-lg flex items-center justify-center shadow-sm"
          >
            <Icon icon="mdi:chip" class="w-7 h-7 text-white" />
          </div>
          <div class="flex-1">
            <h2 class="text-2xl font-bold text-gray-900 mb-1">
              EINAO Tech Fee Statistics
            </h2>
            <p class="text-sm text-gray-600">
              {registryType} Registry — View tech fees and payment volumes
            </p>
          </div>
        </div>
        <button
      ...
      >
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            {#if selectedPeriodType === "date-range"}
              <div class="min-w-0">
                <h3
                  id="techfee-start-date-label"
                  class="text-sm font-semibold text-gray-700 mb-2"
                >
                  Start Date
                </h3>
                <output class="block mb-2 text-sm text-gray-900"
                  >{selectedStartDate?.toString() ?? ""}</output
                >
                <div
                  class="overflow-x-auto border border-gray-200 rounded-lg bg-white"
                >
                  <Calendar
                    aria-labelledby="techfee-start-date-label"
              ...
              <div class="min-w-0">
                <h3
                  id="techfee-end-date-label"
                  class="text-sm font-semibold text-gray-700 mb-2"
                >
                  End Date
                </h3>
                <output class="block mb-2 text-sm text-gray-900"
                  >{selectedEndDate?.toString() ?? ""}</output
                >
                <div
                  class="overflow-x-auto border border-gray-200 rounded-lg bg-white"
                >
                  <Calendar
                    aria-labelledby="techfee-end-date-label"
        ...
        {#if compareMode}
          <div class="flex flex-col justify-start">
            <div
              class="bg-gray-50 border-2 border-blue-300 rounded-lg p-4 h-full flex flex-col gap-3"
            >
              ...
              <tfoot>
                <tr class="bg-slate-100 border-t-2 border-slate-200">
                  ...
                  <td class="py-3.5 px-6 text-right">
                    <span
                      class="inline-flex items-center justify-center px-3 py-1.5 rounded-full text-sm font-bold text-white bg-blue-600"
                    >
                      {formatCurrency(
          ...
          <div class="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p class="text-sm text-slate-500">
                Tech fee per payment type {compareMode
                  ? "across periods"
                  : "for selected period"}
              </p>
            </div>
          ...
          <div class="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 class="font-semibold text-slate-800">
                Tech Fee Share by Payment Type
              </h3>
              <p class="text-sm text-slate-500">
                {compareMode
          ...
          >
            ...
            <div>
              ...
              <p class="text-xs text-slate-400">
                Tech fee per month within the selected quarter{results.periods
                  .length > 1
                  ? "s"
                  : ""}
              </p>
