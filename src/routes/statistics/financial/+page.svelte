<script lang="ts">
  ...
  let selectedStartDate = `${CURRENT_YEAR}-${String(new Date().getMonth() + 1).padStart(2, "0")}-01`;
  let selectedEndDate = new Date().toISOString().slice(0, 10);

  let comparisonPeriods: (FinancePeriodRequestDto & {
    _id: number;
    displayLabel: string;
  })[] = [];
  let nextId = 0;
  ...
  function renderCharts() {
    ...
    const rawLabels = firstPeriod.paymentTypes.map(
      (pt) => pt.paymentType || "Unknown",
    );
      ...
      data: {
        ...
        datasets: results.periods.map((period, index) => ({
          ...
          data: rawLabels.map((rawKey) => {
            const found = period.paymentTypes.find(
              (pt) => (pt.paymentType || "Unknown") === rawKey,
            );
    ...
    const doughnutLabels = firstPeriod.paymentTypes.map((pt) =>
      formatPaymentType(pt.paymentType || "Unknown"),
    );
  ...
  function buildDisplayLabel(period: FinancePeriodRequestDto): string {
    switch (period.Type) {
      ...
      case "year":
        return `${period.Year}`;
      case "month-range":
        return `${MONTHS[(period.StartMonth ?? 1) - 1]}–${MONTHS[(period.EndMonth ?? 6) - 1]} ${period.Year}`;
      case "year-range":
  ...
  function validatePeriodSelection(): boolean {
    if (
      selectedPeriodType === "month-range" &&
  ...
  function addToComparison() {
    ...
    comparisonPeriods = [
      ...comparisonPeriods,
      { ...period, _id: nextId++, displayLabel },
    ];
  ...
  function normalizePaymentTypes(
    paymentTypes: FinancePaymentTypeResultDto[],
  ): FinancePaymentTypeResultDto[] {
    ...
    for (const pt of paymentTypes) {
      ...
      if (map.has(key)) {
        ...
        map.set(key, {
          paymentType: key,
          totalGovernmentFee:
            existing.totalGovernmentFee + pt.totalGovernmentFee,
  ...
  async function fetchSingle() {
    ...
    try {
      const dto = {
        ...
        Periods: [buildCurrentPeriod()],
      };
      const response = await fetch(
        `${baseURL}/api/statistics/finance/compare`,
        {
          method: "POST",
      ...
      if (!response.ok)
        throw new Error(data.error || "Failed to fetch financial statistics");

      // ✅ Normalize payment types after fetch
      if (data.data?.periods) {
        data.data.periods = data.data.periods.map(
          (p: FinancePeriodResultDto) => ({
            ...p,
  ...
  async function fetchComparison() {
    if (comparisonPeriods.length < 2) {
      toast.error("Please add at least 2 periods to compare");
      return;
    }
    if (!validatePeriodSelection()) return;
    loading = true;
  ...
  function handleClearAll() {
    ...
    selectedStartDate = `${CURRENT_YEAR}-${String(new Date().getMonth() + 1).padStart(2, "0")}-01`;
    selectedEndDate = new Date().toISOString().slice(0, 10);
  }

  $: allPaymentTypes = results
    ? [
        ...new Set(
          results.periods.flatMap((p) =>
            p.paymentTypes.map((pt) => pt.paymentType),
          ),
        ),
      ]
    : [];
  ...
  $: maxPaymentTypeFee = results
    ? Math.max(
        ...results.periods.flatMap((p) =>
          p.paymentTypes.map((pt) => pt.totalGovernmentFee),
        ),
        1,
      )
    : 1;
  ...
  function mergePaymentTypes(
    returned: { paymentType: string; totalAmount: number; count: number }[],
  ) {
...
<div class="min-h-screen bg-gray-50">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            {#if selectedPeriodType !== "year-range" && selectedPeriodType !== "date-range"}
              <div>
                ...
                <div class="relative">
                  ...
                  >
                    {#each YEARS as year}<option value={year}>{year}</option
                      >{/each}
                  </select>
                  <Icon
              ...
              <div>
                ...
                <div class="relative">
                  <select
                    ...
                    class="appearance-none w-full bg-white border-2 border-gray-300 rounded-lg px-4 py-2.5 pr-10 text-sm font-medium text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 cursor-pointer transition-all"
                  >
                    {#each MONTHS as month}<option value={month}>{month}</option
                      >{/each}
                  </select>
                  <Icon
                    icon="lucide:chevron-down"
              ...
              <div>
                ...
                <div class="relative">
                  <select
                    ...
                    class="appearance-none w-full bg-white border-2 border-gray-300 rounded-lg px-4 py-2.5 pr-10 text-sm font-medium text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 cursor-pointer transition-all"
                  >
                    {#each MONTHS as month, i}<option value={i + 1}
                        >{month}</option
                      >{/each}
                  </select>
                  <Icon
            ...
            {#if selectedPeriodType === "date-range"}
              <div class="min-w-0">
                ...
                <input
                  ...
                  class="w-full bg-white border-2 border-gray-300 rounded-lg px-4 py-2.5 text-sm font-medium text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all"
                />
                <div
                  class="mt-2 overflow-x-auto border border-gray-200 rounded-lg bg-white"
                >
                  <Calendar
            ...
            >
              <div class="flex items-center gap-3 mb-1">
                ...
                <span class="ml-auto text-xs text-gray-400"
                  >{comparisonPeriods.length}/5</span
                >
              </div>
              <button
    ...
    {#if error}
      <div
        class="bg-red-50 border border-red-200 rounded-lg p-4 mb-6 flex items-center gap-3 text-red-700"
      >
    ...
    {#if results && results.periods.length > 0}
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="bg-slate-50 border-b border-slate-200">
                <th
                  class="text-left py-3 px-6 font-semibold text-slate-600 text-xs uppercase tracking-wide"
                  >Period</th
                >
                <th
                  class="text-left py-3 px-6 font-semibold text-slate-600 text-xs uppercase tracking-wide"
                ...
                >
                <th
                  class="text-right py-3 px-6 font-semibold text-slate-600 text-xs uppercase tracking-wide"
                  >Total Government Fee</th
                >
                <th
                  class="text-right py-3 px-6 font-semibold text-slate-600 text-xs uppercase tracking-wide"
                  >Total Payments</th
            ...
            <tbody>
              {#each results.periods as period, index}
                <tr
                  class="border-b border-slate-100 {index % 2 === 0
                    ? 'bg-white'
                    : 'bg-slate-50/50'} hover:bg-green-50/40 transition-colors"
                >
                  <td class="py-4 px-6">
                  ...
                  <td class="py-4 px-6 text-right">
                    <span
                      class="inline-flex items-center justify-center px-3 py-1.5 rounded-full text-sm font-bold"
                      style="background-color: {COLORS[
                        index % COLORS.length
                      ]}15; color: {COLORS[index % COLORS.length]}"
                    >
                  ...
                  <td class="py-4 px-6 text-right">
                    <span
                      class="inline-flex items-center justify-center px-3 py-1.5 rounded-full text-sm font-semibold"
                      style="background-color: {COLORS[
                        index % COLORS.length
                      ]}15; color: {COLORS[index % COLORS.length]}"
                    >
            ...
            {#if results.periods.length > 1}
              <tfoot>
                <tr class="bg-slate-100 border-t-2 border-slate-200">
                  <td
                    class="py-3.5 px-6 font-bold text-slate-800 text-xs uppercase tracking-wide"
                    colspan="2">Total</td
                  >
                  <td class="py-3.5 px-6 text-right">
                    <span
                      class="inline-flex items-center justify-center px-3 py-1.5 rounded-full text-sm font-bold text-white bg-green-600"
                    >
                      {formatCurrency(
                        results.periods.reduce(
                          (a, p) => a + p.totalGovernmentFee,
                          0,
                        ),
                      )}
                    </span>
                  ...
                  <td class="py-3.5 px-6 text-right">
                    ...
                    >
                      {results.periods
                        .reduce((a, p) => a + p.totalPayments, 0)
                        .toLocaleString()}
                    </span>
                  </td>
                </tr>
          ...
          <div class="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p class="text-sm text-slate-500">
                Government fee per payment type {compareMode
                  ? "across periods"
                  : "for selected period"}
              </p>
            </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-6">
          <div class="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p class="text-sm text-slate-500">
                {compareMode
                  ? `First period: ${results.periods[0].label}`
                  : results.periods[0].label}
              </p>
            </div>
          </div>
          <div class="relative h-64">
      ...
      {#if selectedPeriodType === "quarter" && results.periods.some((p) => p.monthlyBreakdown && p.monthlyBreakdown.length > 0)}
        ...
        >
          ...
          >
            <div
              class="w-9 h-9 bg-green-100 rounded-lg flex items-center justify-center"
            >
              <Icon
                icon="lucide:calendar-days"
            ...
            <div>
              ...
              <p class="text-xs text-slate-400">
                Revenue per month within the selected quarter{results.periods
                  .length > 1
                  ? "s"
                  : ""}
              </p>
                  ...
                  <table class="w-full text-sm">
                    ...
                    <tbody>
                      {#each period.monthlyBreakdown as month, mIndex}
                        {@const pct =
                          period.totalGovernmentFee > 0
                            ? (
                                (month.totalGovernmentFee /
                                  period.totalGovernmentFee) *
                                100
                              ).toFixed(1)
                            : "0.0"}
                        <tr
                          class="border-b border-slate-100 transition-colors {mIndex %
                            2 ===
                          0
                            ? 'bg-white'
                            : 'bg-slate-50/50'} hover:bg-green-50/40"
                        >
                          ...
                          <td class="py-3.5 px-6 text-slate-500 text-xs">
                            {formatDate(month.startDate)} — {formatDate(
                              month.endDate,
                            )}
                          </td>
                          <td class="py-3.5 px-6 text-right">
                            <span
                              ...
                              style="background-color: {COLORS[
                                pIndex % COLORS.length
                              ]}15; color: {COLORS[pIndex % COLORS.length]}"
                            >
                          ...
                          <td class="py-3.5 px-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                              <div
                                class="w-16 bg-gray-100 rounded-full h-1.5 overflow-hidden"
                              >
                                <div
                                  class="h-full rounded-full"
                                  style="width: {pct}%; background-color: {COLORS[
                                    pIndex % COLORS.length
                                  ]}"
                                ></div>
                              </div>
                              <span
                                ...
                                style="color: {COLORS[pIndex % COLORS.length]}"
                                >{pct}%</span
                              >
                            </div>
                          </td>
                    ...
                    <tfoot>
                      <tr class="bg-slate-100 border-t-2 border-slate-200">
                        <td
                          class="py-3.5 px-6 font-bold text-slate-800 text-xs uppercase tracking-wide"
                          colspan="2">Quarter Total</td
                        >
                        <td class="py-3.5 px-6 text-right">
                          <span
                            class="inline-flex items-center justify-center px-2.5 py-1 rounded-full text-xs font-bold text-white"
                            style="background-color: {COLORS[
                              pIndex % COLORS.length
                            ]}"
                          >
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <p class="text-xs text-slate-400">
              {allPaymentTypes.length} payment types
            </p>
          </div>
        </div>
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead>
              <tr class="bg-slate-50 border-b border-slate-200">
                ...
                {#each results.periods as period, index}
                  <th
                    class="text-right py-3 px-6 font-semibold text-xs uppercase tracking-wide"
            ...
            <tbody>
              {#each allPaymentTypes as paymentType, rowIndex}
                <tr
                  class="border-b border-slate-100 transition-colors {rowIndex %
                    2 ===
                  0
                    ? 'bg-white'
                    : 'bg-slate-50/50'} hover:bg-green-50/40"
                >
                  <td class="py-3.5 px-6 font-medium text-slate-700">
                    <div class="flex items-center gap-2">
                      ...
                      {formatPaymentType(paymentType) || "Unknown"}
                      <!-- ✅ formatted -->
                    </div>
                  </td>
                  {#each results.periods as period, index}
                    {@const pt = period.paymentTypes.find(
                  ...
                  <td class="py-3.5 px-6 text-right">
                    <span class="inline-flex flex-col items-end">
                      ...
                      <span class="text-xs text-slate-500 mt-0.5"
                        >{period.totalPayments} payments</span
                      >
                    </span>
                  </td>
                {/each}
