<script lang="ts">
  ...
  async function assignApplication() {
    ...
    if (!assignForm) {
      assignForm = (
        await import("../home/dashboard/components/AssignForm.svelte")
      ).default;
  ...
  let showUpdateStatusForm = false;
  ...
  async function updateForThis() {
    ...
    if (response.ok) {
      ...
      toast.error("Failed to update the correspondence information", {
        position: "top-right",
      });
    }
  ...
  async function ShowCertificatePayment() {
    ...
    if (!history0) {
      toast.error("No application history available for certificate payment", {
        position: "top-right",
      });
      return;
    }
  ...
  let showStatusUpdateForm: boolean = false;
  interface StatusChange {
    newStatus: string | ApplicationStatuses | null;
    userId: string;
    fileId: string | null;
    reason: string;
  }
  ...
  async function confirmChange() {
    isLoading = true;
    statusUpdate.userId = $loggedInUser?.id ?? "";
    statusUpdate.fileId = fileData?.fileId ?? null;

    try {
      await changeFileStatus(statusUpdate);
      step = 2;
    } catch (err) {
      console.error(err);
    } finally {
...
<Dialog.Root bind:open={showCorrespondenceRequest}>
  <Dialog.Content>
    ...
    {:else}
      <p>
        This application has some details missing in the correspondence section,
        this information is needed for payment generation and validation. Should
        your default correspondence be used for this application only, or for
        all your applications with missing or incomplete correspondence
        sections. The default correspondence section can be found in the profile
      </p>
  ...
  <Dialog.Content class="max-w-lg p-6 rounded-lg bg-white shadow-lg">
    ...
    {#if step === 1}
      <div class="flex flex-col gap-4">
        <p>
          ...
          <span class="border rounded p-2 ml-2">{statusUpdate.newStatus}</span>?
        </p>
    ...
    {#if step === 2}
      <div class="flex flex-col items-center justify-center gap-4">
        <p class="text-green-600 font-semibold">Status Updated Successfully!</p>
  ...
  <div class="px-1.5 py-1.5 gap-4">
    ...
    <Card.Root>
      <div class="flex justify-between p-4">
        <Card.Title class="col-span-1 col-start-1"
          >Filing Information</Card.Title
        >
            ...
            <DropdownMenu.Content>
              <DropdownMenu.Group>
                ...
                {#if fileData.fileStatus === ApplicationStatuses.AwaitingCertification}
                  <DropdownMenu.Item
                    on:click={async () => ShowCertificatePayment()}
              ...
              {#if $loggedInUser?.userRoles.includes(UserRoles.Tech)}
                <DropdownMenu.Item
                  on:click={async () => await showOwnershipForm()}
                >
                  Change Ownership
                </DropdownMenu.Item>
              {/if}
              {#if $loggedInUser?.userRoles.includes(UserRoles.SuperAdmin || UserRoles.TrademarkRegistrar || UserRoles.PatentDesignRegistrar)}
                <DropdownMenu.Item
                  on:click={() => (showStatusUpdateForm = true)}
                >
                  Change File Status
                </DropdownMenu.Item>
              {/if}
