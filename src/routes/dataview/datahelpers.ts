...
export function getHistoryData(revision: RevisionsType | null) {
  ...
  const statuses = Object.values(ApplicationStatuses).filter(
    (v): v is ApplicationStatuses => typeof v === "number",
  );
  for (let i = 0; i < 10; i++) {
    ...
    const selectedBefore: (ApplicationStatuses | null)[] = [beforeStatus, null];
    const selectedAfter: (ApplicationStatuses | null)[] = [afterStatus, null];
...
export function getAppHistories() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    dataList.push({
      ...
      applicationDate: faker.date.soon().toString().split("GMT")[0],
      expiryDate: faker.date.soon().toString().split("GMT")[0],
      currentStatus: afterStatus,
      applicationLetters: null,
      paymentId: faker.database.mongodbObjectId(),
      oldValue: faker.lorem.paragraph(5),
      newValue: faker.lorem.paragraph(5),
      fieldToChange: "patentAbstract",
      statusHistory: null,
    });
  }
  return dataList;
...
export function CanTreatApplication(
  ...
  if (applicationStatus === ApplicationStatuses.AwaitingRenewalConfirmation) {
    if (type === FilingType.Patent || type === FilingType.Design) {
      hasRole = userRoles.some((x) =>
        [
          UserRoles.SuperAdmin,
