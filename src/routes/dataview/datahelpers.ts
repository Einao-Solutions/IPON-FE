...
export function getStatuses(
	...
	if (fileType === FilingType.Trademark && currentStatus == ApplicationStatuses.AwaitingExaminer) {
		return [
	...
	if (fileType === FilingType.Trademark && currentStatus == ApplicationStatuses.Rejected) {
		return [
			ApplicationStatuses.Publication
			// ApplicationStatuses.Re_conduct
		];
	...
	if (
		...
		[ApplicationStatuses.AwaitingExaminer, ApplicationStatuses.KivExaminer].includes(currentStatus)
	) {
		const values = [
...
export function getRevisionStatuses(currentStatus: ApplicationStatuses): ApplicationStatuses[] {
	...
	switch (currentStatus) {
		...
		case ApplicationStatuses.KivExaminer:
		case ApplicationStatuses.Rejected:
			responses = [ApplicationStatuses.Re_conduct, ApplicationStatuses.Publication];
			break;
		case ApplicationStatuses.AwaitingExaminer:
			responses = [
...
export function showTreatUpdateAppButton(
	...
	let hasRequiredPatentSearchPatentRoles = [UserRoles.PatentSearch, UserRoles.Support];
	let hasRequiredTrademarkSearchPatentRoles = [UserRoles.TrademarkSearch, UserRoles.Support];
	let hasRequiredDesignSearchPatentRoles = [UserRoles.DesignSearch, UserRoles.Support];
	let hasRequiredPatentExamRoles = [UserRoles.PatentExaminer, UserRoles.Support];
	let hasRequiredTrademarkExamRoles = [UserRoles.TrademarkExaminer, UserRoles.Support];
	let hasRequiredDesignExamRoles = [UserRoles.DesignExaminer, UserRoles.Support];
	let searchOffice = [
		ApplicationStatuses.KivSearch,
		ApplicationStatuses.FormalityFail,
		ApplicationStatuses.AwaitingSearch,
	...
	if (
		...
		userroles.filter((x) => hasRequiredPatentSearchPatentRoles.includes(x)).length >= 1
	) {
		return true;
	}
	if (
	...
	if (
		...
		searchOffice.includes(currentStatus) &&
		userroles.filter((x) => hasRequiredDesignSearchPatentRoles.includes(x)).length >= 1
	) {
		return true;
	}
	if (
	...
	if (
		...
		searchOffice.includes(currentStatus) &&
		userroles.filter((x) => hasRequiredTrademarkSearchPatentRoles.includes(x)).length >= 1
	) {
		return true;
	}
	return (
		applicationType === FilingType.Trademark &&
		examinerOffice.includes(currentStatus) &&
		userroles.filter((x) => hasRequiredTrademarkExamRoles.includes(x)).length >= 1
	);
}
...
export function CanTreatApplication(
	...
	if (
		...
		if (type === FilingType.Patent) {
			hasRole = userRoles.some((x) => [UserRoles.PatentSearch, UserRoles.Support].includes(x));
		}

		if (type === FilingType.Trademark) {
			hasRole = userRoles.some((x) => [UserRoles.TrademarkSearch, UserRoles.Support].includes(x));
		}
	}
	...
	if (
		...
		if (type === FilingType.Patent) {
			hasRole = userRoles.some((x) => [UserRoles.PatentExaminer, UserRoles.Support].includes(x));
		}
		...
		if (type == FilingType.Trademark) {
			hasRole = userRoles.some((x) => [UserRoles.TrademarkExaminer, UserRoles.Support].includes(x));
		}
	}
	if (applicationStatus === ApplicationStatuses.Rejected) {
		if (type === FilingType.Patent) {
			hasRole = userRoles.some((x) =>
				[UserRoles.PatentExaminer, UserRoles.Support, UserRoles.AppealExaminer].includes(x)
			);
		}
		...
		if (type == FilingType.Trademark) {
			hasRole = userRoles.some((x) =>
				[UserRoles.TrademarkExaminer, UserRoles.Support, UserRoles.AppealExaminer].includes(x)
			);
		}
	}
	if (
...
export function parseLoggedInUser(cookie: string) {
	const cookieUser = cookie.split(';').find((x) => x.startsWith(' user=') || x.startsWith('user='));
	if (!cookieUser) {
		return undefined;
	}
