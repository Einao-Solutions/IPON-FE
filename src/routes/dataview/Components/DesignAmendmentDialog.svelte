<script lang="ts">
  ...
  async function fetchAmendmentDetails() {
    ...
    try {
      const response = await fetch(
        `${baseURL}/api/files/GetDesignAmendmentDetails?fileId=${encodeURIComponent(fileId)}&appId=${encodeURIComponent(applicationId)}`
      );
  ...
  async function handleAmendmentDecision(approve: boolean) {
    ...
    try {
      ...
      const response = await fetch(
        `${baseURL}/api/files/DesignAmendmentDecision`,
        {
          method: "POST",
          headers: {
      ...
      const successMessage = approve
        ? "Design amendment has been successfully approved."
        : "Design amendment has been successfully rejected.";
  ...
  function getAmendmentTypeLabel(type: string): string {
    switch (type) {
      ...
      case 'FileTitle': return 'Title of Design Change';
  ...
  >
    <Dialog.Header class="flex-shrink-0">
      <Dialog.Title class="text-2xl font-bold">
        Design Amendment Details
      </Dialog.Title>
      <Dialog.Description>
        Review and process design amendment details.
      </Dialog.Description>
    </Dialog.Header>
          ...
          <div class="bg-purple-50 rounded-lg p-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p class="mt-1 p-2 bg-white rounded border">
                  {#if amendmentDetails.filingDate}
                    {(() => { const [d, m, y] = amendmentDetails.filingDate.split(/[/ :]/); return new Date(+y, +m - 1, +d).toLocaleDateString(); })()}
                  {:else}
                    N/A
                  {/if}
                </p>
                      ...
                      <div>
                        ...
                        <div class="space-y-4">
                          {#each amendmentDetails.changes.newApplicantData.EditedApplicants as editedApplicant, index}
                            {@const oldApplicant = findById(amendmentDetails.changes.oldApplicants || [], editedApplicant.id)}
                            <div>
                      ...
                      <div class="border rounded-lg p-4 bg-red-50">
                        ...
                        <div class="space-y-3">
                          {#each amendmentDetails.changes.newApplicantData.RemoveIds as removedId}
                            {@const removedApplicant = findById(amendmentDetails.changes.oldApplicants || [], removedId)}
                            {#if removedApplicant}
