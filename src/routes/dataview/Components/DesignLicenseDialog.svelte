<script lang="ts">
  ...
  async function fetchLicenseDetails() {
    ...
    try {
      const response = await fetch(
        `${baseURL}/api/files/GetDesignLicenseDetails?fileId=${encodeURIComponent(fileId)}`
      );
  ...
  async function handleLicenseDecision(approve: boolean) {
    ...
    try {
      ...
      const successMessage = approve ? "Design license has been successfully approved." : "Design license has been successfully rejected.";
  ...
  >
    <Dialog.Header class="flex-shrink-0">
      <Dialog.Title class="text-2xl font-bold flex items-center gap-2">
        Design License Details
      </Dialog.Title>
      <Dialog.Description>
        Review and process design license application details.
      </Dialog.Description>
    </Dialog.Header>
