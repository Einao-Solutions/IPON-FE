<script lang="ts">
  ...
  async function fetchAssignmentDetails() {
    ...
    try {
      const response = await fetch(
        `${baseURL}/api/files/GetDesignAssignmentDetails?fileId=${encodeURIComponent(fileId)}`
      );
  ...
  async function handleAssignmentDecision(approve: boolean) {
    ...
    try {
        ...
        `${baseURL}/api/files/DesignAssignmentDecision`, 
        {
          ...
          body: JSON.stringify({
            ...
            newAssignee: approve && assignmentDetails?.newAssignee ? {
              ...
              City: assignmentDetails.newAssignee.city,
              State: assignmentDetails.newAssignee.state,
              Country: assignmentDetails.newAssignee.nationality
            } : null,
          }),
        }
      );
      ...
      const successMessage = approve ? "Design assignment has been successfully approved." : "Design assignment has been successfully rejected.";
...
<Dialog.Root bind:open>
  ...
  >
    <Dialog.Header class="flex-shrink-0">
      ...
      <Dialog.Description>
        Review and process design assignment application details.
      </Dialog.Description>
    </Dialog.Header>
        ...
        <div class="space-y-6">
          ...
          <div class="mb-6">
            <Label class="font-semibold mb-3 block flex items-center gap-2">
              ...
              Deed of Assignment Attachments:
            </Label>
            {#if assignmentDetails.deedOfAssignmentAttachments && Array.isArray(assignmentDetails.deedOfAssignmentAttachments) && assignmentDetails.deedOfAssignmentAttachments.length > 0}
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                {#each assignmentDetails.deedOfAssignmentAttachments as attachmentGroup}
                        ...
                        <div class="flex items-center justify-between gap-3">
                          <div class="flex-1 min-w-0">
                            <div class="font-medium text-gray-800 truncate">
                              {attachmentGroup.name || "Deed of Assignment"} - Document {index + 1}
                            </div>
            ...
            {:else}
              <div class="text-center py-6 text-gray-500 bg-gray-50 rounded-lg border-2 border-dashed">
                ...
                <p>No deed of assignment attachments submitted</p>
              </div>
            {/if}
          </div>
          ...
          <div class="mb-6">
            ...
            {#if assignmentDetails.supportingDocumentAttachments && Array.isArray(assignmentDetails.supportingDocumentAttachments) && assignmentDetails.supportingDocumentAttachments.length > 0}
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                {#each assignmentDetails.supportingDocumentAttachments as attachmentGroup}
                  {#if attachmentGroup.url && Array.isArray(attachmentGroup.url)}
                    {#each attachmentGroup.url as fileUrl, index}
                      <div class="border rounded-lg p-3 bg-gray-100 hover:bg-gray-200 transition-colors">
                        <div class="flex items-center justify-between gap-3">
