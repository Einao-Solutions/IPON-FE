<script lang="ts">
  ...
  async function fetchMortgageDetails() {
    ...
    try {
      const response = await fetch(
        `${baseURL}/api/files/GetDesignMortgageDetails?fileId=${encodeURIComponent(fileId)}`
      );
  ...
  async function handleMortgageDecision(approve: boolean) {
    ...
    try {
      ...
      const response = await fetch(
        `${baseURL}/api/files/DesignMortgageDecision`, 
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            newMortgagee: approve && mortgageDetails?.newMortgagee ? {
              ...
              Country: mortgageDetails.newMortgagee.nationality
            } : null,
          }),
        }
      );
      ...
      const successMessage = approve ? "Design mortgage has been successfully approved." : "Design mortgage has been successfully rejected.";
  ...
  >
    <Dialog.Header class="flex-shrink-0">
      <Dialog.Title class="text-2xl font-bold flex items-center gap-2">
        Design Mortgage Details
      </Dialog.Title>
      <Dialog.Description>
        Review and process design mortgage application details.
      </Dialog.Description>
    </Dialog.Header>
        ...
        <div class="space-y-6">
          ...
          <div class="mb-6">
            ...
            {#if mortgageDetails.deedOfMortgageAttachments && Array.isArray(mortgageDetails.deedOfMortgageAttachments) && mortgageDetails.deedOfMortgageAttachments.length > 0}
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                {#each mortgageDetails.deedOfMortgageAttachments as attachmentGroup}
                        ...
                        <div class="flex items-center justify-between gap-3">
                          <div class="flex-1 min-w-0">
                            <div class="font-medium text-gray-800 truncate">
                              {attachmentGroup.name || "Deed of Mortgage"} - Document {index + 1}
                            </div>
            ...
            {:else}
              <div class="text-center py-6 text-gray-500 bg-gray-50 rounded-lg border-2 border-dashed">
                ...
                <p>No deed of mortgage attachments submitted</p>
              </div>
            {/if}
          </div>
          ...
          <div class="mb-6">
            ...
            {#if mortgageDetails.supportingDocumentAttachments && Array.isArray(mortgageDetails.supportingDocumentAttachments) && mortgageDetails.supportingDocumentAttachments.length > 0}
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                {#each mortgageDetails.supportingDocumentAttachments as attachmentGroup}
                  {#if attachmentGroup.url && Array.isArray(attachmentGroup.url)}
                    {#each attachmentGroup.url as fileUrl, index}
                      <div class="border rounded-lg p-3 bg-gray-100 hover:bg-gray-200 transition-colors">
                        <div class="flex items-center justify-between gap-3">
