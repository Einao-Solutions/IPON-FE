        ...
        appType:
          summaries.length > 1
            ...
            : first?.applicationType
              ? mapTypeToString(first.applicationType)
              : null,
      });
    }
    dataList = [..._dataList];
...
<div>
  <div class="flex items-center py-4">
    ...
    <div
      class="ml-auto mr-10 {($selectedFilesForAction?.length ?? 0 >= 1)
        ? ''
        : 'hidden'}"
    >
      <DropdownMenu.Root>
        ...
        <DropdownMenu.Content>
          <DropdownMenu.Item on:click={() => showSelections()}
            >Create ticket</DropdownMenu.Item
          >
    ...
    {#if showRenew}
      <Button class="mx-2 bg-green-600" on:click={() => batchRenew()}
        >Renew all</Button
      >
    {/if}
  </div>
                        ...
                        {#if getAppStatus(row.id)}
                          <div class="grid grid-cols-1">
                            ...
                            <div
                              ...
                              style="background-color:{getStatusColour(
                                cell.render()
                              )}"
                            >
                              <Render of={cell.render()} />
