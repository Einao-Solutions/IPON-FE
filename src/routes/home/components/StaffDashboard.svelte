<script lang="ts">
    ...
    function getApplicationTypeIcon(appType: number) {
        ...
        switch (appType) {
            ...
            case 2: return { svg: '', color: uniformColor, bgColor: uniformBgColor };
            default: return { svg: '', color: uniformColor, bgColor: uniformBgColor };
        }
    }
    ...
    function getRegistryHubTitle() {
        ...
        switch (primaryFileType) {
            case FileTypes.Trademark: return 'Trademark Registry Hub';
            case FileTypes.Patent: return 'Patent Registry Hub';
            case FileTypes.Design: return 'Design Registry Hub';
            default: return 'IP Registry Hub';
        }
    }
</script>
...
{:else}
        ...
        <div class="space-y-4">
            <Accordion.Root class="space-y-3">
                {#each getType(FileTypes.Trademark) as item}
                    ...
                    <Accordion.Item value="tm-{item.type}" class="border border-slate-200/60 rounded-xl bg-gradient-to-r from-white via-slate-50/50 to-white mb-2 hover:shadow-lg hover:shadow-green-500/10 transition-all duration-300 hover:scale-[1.01] hover:border-green-300/60">
    ...
    <div class="{isPatentRelated() ? '' : 'hidden'} mt-3">
        {#if isPatentRelated() && isDesignRelated()}
            ...
            <Accordion.Root class="space-y-3 mb-3">
                <Accordion.Item value="patent-hub" class="border border-green-200/60 rounded-xl bg-gradient-to-r from-white via-green-50/30 to-white hover:shadow-lg hover:shadow-green-500/10 transition-all duration-300 hover:border-green-300/60">
                    <Accordion.Trigger class="hover:bg-transparent data-[state=open]:bg-transparent [&>div]:no-underline">
                        <div class="flex items-center justify-between w-full px-3 py-2">
                            <div class="flex items-center space-x-3 flex-1">
                                ...
                                <div class="flex-1 text-left">
                                    ...
                                    <p class="text-xs text-gray-500">Click to view patent application types and statuses</p>
                                </div>
                            </div>
                        </div>
                    </Accordion.Trigger>
                        ...
                        <div class="px-4 pb-4 space-y-3">
                            <Accordion.Root class="space-y-3">
                                {#each getType(FileTypes.Patent) as item}
                                    {@const iconInfo = getApplicationTypeIcon(parseInt(item.type))}
                                    <Accordion.Item value="patent-{item.type}" class="border border-slate-200/60 rounded-xl bg-gradient-to-r from-white via-slate-50/50 to-white mb-2 hover:shadow-lg hover:shadow-green-500/10 transition-all duration-300 hover:scale-[1.01] hover:border-green-300/60">
                                        <Accordion.Trigger class="hover:bg-transparent data-[state=open]:bg-transparent [&>div]:no-underline">
                                            <div class="flex items-center justify-between w-full px-3 py-2">
                                                <div class="flex items-center space-x-3 flex-1">
                                                    <div class="w-10 h-10 bg-gradient-to-br {iconInfo.bgColor} rounded-lg flex items-center justify-center shadow-sm border border-green-100">
        ...
        {:else}
            ...
            <div class="space-y-4">
                <Accordion.Root class="space-y-3">
                    {#each getType(FileTypes.Patent) as item}
                        {@const iconInfo = getApplicationTypeIcon(parseInt(item.type))}
                        <Accordion.Item value="p-{item.type}" class="border border-slate-200/60 rounded-xl bg-gradient-to-r from-white via-slate-50/50 to-white mb-2 hover:shadow-lg hover:shadow-green-500/10 transition-all duration-300 hover:scale-[1.01] hover:border-green-300/60">
                            <Accordion.Trigger class="hover:bg-transparent data-[state=open]:bg-transparent [&>div]:no-underline">
                                <div class="flex items-center justify-between w-full px-3 py-2">
                                    <div class="flex items-center space-x-3 flex-1">
                                        <div class="w-10 h-10 bg-gradient-to-br {iconInfo.bgColor} rounded-lg flex items-center justify-center shadow-sm border border-green-100">
    ...
    <div class="{isDesignRelated() ? '' : 'hidden'} mt-3">
        {#if isPatentRelated() && isDesignRelated()}
            <!-- PatentDesignRegistrar / SuperAdmin / Tech — wrap in outer accordion -->
            <Accordion.Root class="space-y-3 mb-3">
                <Accordion.Item value="design-hub" class="border border-green-200/60 rounded-xl bg-gradient-to-r from-white via-green-50/30 to-white hover:shadow-lg hover:shadow-green-500/10 transition-all duration-300 hover:border-green-300/60">
                    <Accordion.Trigger class="hover:bg-transparent data-[state=open]:bg-transparent [&>div]:no-underline">
                        <div class="flex items-center justify-between w-full px-3 py-2">
                            <div class="flex items-center space-x-3 flex-1">
                                <div class="w-10 h-10 bg-green-100 rounded-lg flex items-center justify-center shadow-sm border border-green-200">
                                ...
                                <div class="flex-1 text-left">
                                    ...
                                    <p class="text-xs text-gray-500">Click to view design application types and statuses</p>
                                </div>
                            </div>
                        </div>
                    </Accordion.Trigger>
                        ...
                        <div class="px-4 pb-4 space-y-3">
                            <Accordion.Root class="space-y-3">
                                {#each getType(FileTypes.Design) as item}
                                    {@const iconInfo = getApplicationTypeIcon(parseInt(item.type))}
                                    <Accordion.Item value="design-{item.type}" class="border border-slate-200/60 rounded-xl bg-gradient-to-r from-white via-slate-50/50 to-white mb-2 hover:shadow-lg hover:shadow-green-500/10 transition-all duration-300 hover:scale-[1.01] hover:border-green-300/60">
                                        <Accordion.Trigger class="hover:bg-transparent data-[state=open]:bg-transparent [&>div]:no-underline">
                                            <div class="flex items-center justify-between w-full px-3 py-2">
                                                <div class="flex items-center space-x-3 flex-1">
                                                    <div class="w-10 h-10 bg-gradient-to-br {iconInfo.bgColor} rounded-lg flex items-center justify-center shadow-sm border border-green-100">
        ...
        {:else}
            ...
            <div class="space-y-4">
                <Accordion.Root class="space-y-3">
                    {#each getType(FileTypes.Design) as item}
                        {@const iconInfo = getApplicationTypeIcon(parseInt(item.type))}
                        <Accordion.Item value="d-{item.type}" class="border border-slate-200/60 rounded-xl bg-gradient-to-r from-white via-slate-50/50 to-white mb-2 hover:shadow-lg hover:shadow-green-500/10 transition-all duration-300 hover:scale-[1.01] hover:border-green-300/60">
                            <Accordion.Trigger class="hover:bg-transparent data-[state=open]:bg-transparent [&>div]:no-underline">
                                <div class="flex items-center justify-between w-full px-3 py-2">
                                    <div class="flex items-center space-x-3 flex-1">
                                        <div class="w-10 h-10 bg-gradient-to-br {iconInfo.bgColor} rounded-lg flex items-center justify-center shadow-sm border border-green-100">
