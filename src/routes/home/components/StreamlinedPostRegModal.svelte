<script lang="ts">
	...
	function getPostRegistrationRoute(serviceId: string, fileId: string, fileType: string): string {
	...
	async function handleSearch(): Promise<void> {
		...
		try {
			...
			const res = await fetch(`${baseURL}/api/files/files/${encodeURIComponent(searchQuery.trim())}/type`);
	...
	async function handleRenewalService(fileNumber: string, ipType: string): Promise<void> {
		try {
			// Get file details - same API call as postregistration search
			const response = await fetch(`${baseURL}/api/files/GetFileByFileNumber?fileNumber=${fileNumber}`);
			...
			if (isPatent) {
				...
				if (['pct', 'conventional'].includes(patentTypeStr)) {
					if (!Array.isArray(fileResult.firstPriorityInfo) || fileResult.firstPriorityInfo.length === 0) {
						error = 'First Priority Information does not exist for this file. Please use the Update Patent module on the dashboard to update your file before filing for a renewal.';
						return;
					}
				}
				...
				const currentStatus = fileResult?.fileStatus;
				if (currentStatus === '20') { // AwaitingPayment status
	...
	>
		<div class="bg-white rounded-lg p-6 w-full max-w-md mx-4" on:click|stopPropagation>
			<div class="flex items-center justify-between mb-4">
				<h3 id="modal-title" class="text-lg font-bold text-black">{serviceName}</h3>
				<button
					type="button"
