<script lang="ts">
  ...
  function getTotal(type: FileTypes) {
    return (
      userDashStats?.fileStats.find((x) => x.fileType === type)?.count ?? 0
    );
  }
  ...
  function getType(fileType: FileTypes) {
    ...
    const val =
      userDashStats?.detailedStats?.filter((x) => x.fileType === fileType) ??
      [];
    ...
    const mapped = Object.entries(grouped).map(([type, items]) => ({
      type,
      items,
    }));
  ...
  function getApplicationTypeIcon(appType: number) {
    ...
    switch (appType) {
      case 0: // NewApplication
        return {
          svg: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>',
          color: uniformColor,
          bgColor: uniformBgColor,
        };
      case 1: // LicenseRenewal
        return {
          svg: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>',
          color: uniformColor,
          bgColor: uniformBgColor,
        };
      case 2: // DataUpdate
        return {
          svg: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path>',
          color: uniformColor,
          bgColor: uniformBgColor,
        };
      case 3: // Recapture
        return {
          svg: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2H5a2 2 0 00-2-2z"></path>',
          color: uniformColor,
          bgColor: uniformBgColor,
        };
      case 4: // None
      ...
      case 5: // Assignment
        return {
          svg: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"></path>',
          color: uniformColor,
          bgColor: uniformBgColor,
        };
      case 6: // Ownership
        return {
          svg: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v-2L7.257 13.243A6 6 0 0113 5a2 2 0 012 2z"></path>',
          color: uniformColor,
          bgColor: uniformBgColor,
        };
      ...
      case 8: // Merger
        return {
          svg: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path>',
          color: uniformColor,
          bgColor: uniformBgColor,
        };
      case 9: // ChangeOfName
      ...
      case 13: // AppealRequest
        return {
          svg: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16l-3-3m3 3l3-3"></path>',
          color: uniformColor,
          bgColor: uniformBgColor,
        };
      case 14: // PublicationStatusUpdate
        return {
          svg: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z"></path>',
          color: uniformColor,
          bgColor: uniformBgColor,
        };
      case 15: // WithdrawalRequest
        return {
          svg: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"></path>',
          color: uniformColor,
          bgColor: uniformBgColor,
        };
      case 16: // NewOpposition
        return {
          svg: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.618 5.984A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016zM12 9v2m0 4h.01"></path>',
          color: uniformColor,
          bgColor: uniformBgColor,
        };
      case 17: // Amendment
  ...
  {:else}
    {#if !showOnlyStatistics}
      <div class="space-y-3">
        <!-- Trademark Total -->
        <a
          href={getFileUrl("fileType=2&titleType=specific")}
          class="group flex items-center justify-between p-4 bg-gradient-to-r from-green-50 via-white to-green-50 border border-green-200/40 rounded-xl hover:shadow-lg hover:shadow-green-500/20 transition-all duration-300 hover:scale-[1.01] hover:border-green-300/60 relative overflow-hidden"
        >
          <div class="flex items-center space-x-4">
            <div
              class="w-10 h-10 bg-gradient-to-br from-green-100 via-green-50 to-emerald-50 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300 shadow-sm"
            >
              <Icon
                icon="mdi:scale-balance"
                class="text-lg text-green-800 group-hover:text-green-700"
              />
            </div>
            <div>
              <p
                class="font-semibold text-slate-800 group-hover:text-slate-900"
              >
          ...
          <div class="text-right">
            <p
              class="text-2xl font-bold text-slate-800 group-hover:text-green-700 transition-colors duration-300"
            >
              {getTotal(FileTypes.Trademark).toLocaleString()}
            </p>
            <div class="flex items-center justify-end mt-1">
              <Icon
                icon="heroicons:arrow-right"
                class="text-slate-400 group-hover:text-green-500 text-sm transition-colors duration-300"
              />
            </div>
          </div>
        </a>
        ...
        <!-- Patent Total -->
        <a
          href={getFileUrl("fileType=0&titleType=specific")}
          class="group flex items-center justify-between p-4 bg-gradient-to-r from-green-50 via-white to-green-50 border border-green-200/40 rounded-xl hover:shadow-lg hover:shadow-green-500/20 transition-all duration-300 hover:scale-[1.01] hover:border-green-300/60 relative overflow-hidden"
        >
          <div class="flex items-center space-x-4">
            <div
            ...
            <div>
              ...
              <p class="text-xs text-slate-500">
                Protected inventions and innovations
              </p>
            </div>
          </div>
          <div class="text-right">
            <p
              class="text-2xl font-bold text-slate-800 group-hover:text-green-700 transition-colors duration-300"
            >
              {getTotal(FileTypes.Patent).toLocaleString()}
            </p>
            <div class="flex items-center justify-end mt-1">
              <Icon
                icon="heroicons:arrow-right"
        ...
        <!-- Design Total -->
        <a
          href={getFileUrl("fileType=1&titleType=specific")}
          class="group flex items-center justify-between p-4 bg-gradient-to-r from-green-50 via-white to-green-50 border border-green-200/40 rounded-xl hover:shadow-lg hover:shadow-green-500/20 transition-all duration-300 hover:scale-[1.01] hover:border-green-300/60 relative overflow-hidden"
        >
          <div class="flex items-center space-x-4">
            <div
          ...
          <div class="text-right">
            <p
              class="text-2xl font-bold text-slate-800 group-hover:text-green-700 transition-colors duration-300"
            >
              {getTotal(FileTypes.Design).toLocaleString()}
            </p>
            <div class="flex items-center justify-end mt-1">
              <Icon
                icon="heroicons:arrow-right"
        ...
        >
          <div class="flex items-center space-x-4">
            <div
              class="w-10 h-10 bg-gradient-to-br from-orange-100 via-orange-50 to-amber-50 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300 shadow-sm"
            >
              <Icon
                icon="mdi:gavel"
                class="text-lg text-orange-700 group-hover:text-orange-600"
              />
            </div>
            <div>
              <p
              ...
              <p class="text-xs text-slate-500">
                Filed oppositions and related applications
              </p>
            </div>
          </div>
          <div class="text-right">
            <div class="flex items-center justify-end mt-1">
              <Icon
                icon="heroicons:arrow-right"
                class="text-slate-400 group-hover:text-orange-500 text-sm transition-colors duration-300"
              />
            </div>
          </div>
        </a>
                  ...
                  <div class="border rounded-md p-2">
                    {#each item.items as appInfo}
                      <a
                        ...
                        href={getFileUrl(
                          `fileType=0&appType=${parseInt(item.type)}&status=${appInfo.status}&titleType=custom`,
                        )}
                      >
                  ...
                  <div class="border rounded-md p-2">
                    {#each item.items as appInfo}
                      <a
                        class="flex space-x-2 hover:bg-gray-500 my-1.5 items-center justify-between"
                        href={getFileUrl(
                          `fileType=1&appType=${parseInt(item.type)}&status=${appInfo.status}&titleType=custom`,
                        )}
                      >
                        <AppStatusTag value={appInfo.status} />
                        <p class="">{appInfo.count}</p>
            ...
            >
              <div class="flex items-center justify-between w-full px-3 py-2">
                <div class="flex items-center space-x-3 flex-1">
                  <div
                    class="w-10 h-10 bg-gradient-to-br from-green-100 via-green-50 to-emerald-50 rounded-lg flex items-center justify-center shadow-sm border border-green-100"
                  >
                    <Icon
                      icon="mdi:scale-balance"
                  ...
                  <div class="flex-1 text-left">
                    <div class="flex items-center gap-2 mb-0.5">
                      ...
                      <span
                        ...
                        >{getType(FileTypes.Trademark).length}</span
                      >
                    </div>
                    <p class="text-xs text-slate-500">
                      Registered brand identities
                    </p>
                  </div>
                </div>
              ...
              <div class="px-2 pb-3">
                <Accordion.Root class="space-y-2">
                  {#each getType(FileTypes.Trademark) as item}
                    ...
                    <Accordion.Item
                      value="trademark-{item.type}"
                      class="border border-slate-200/40 rounded-lg bg-white/80 hover:shadow-md hover:shadow-green-500/5 transition-all duration-200"
                    >
                      <Accordion.Trigger
                        class="hover:bg-transparent data-[state=open]:bg-transparent [&>div]:no-underline"
                      >
                        ...
                        >
                          <div class="flex items-center space-x-2 flex-1">
                            <div
                              class="w-8 h-8 bg-gradient-to-br {iconInfo.bgColor} rounded-md flex items-center justify-center shadow-sm border border-green-100"
                            >
                              <div
                                class="w-3 h-3 {iconInfo.color} font-bold text-sm flex items-center justify-center"
                              >
                            ...
                            <div class="flex-1 text-left">
                              <div class="flex items-center gap-2">
                                ...
                                <span
                                  class="bg-green-50 text-green-800 px-1 py-0.5 rounded text-xs font-medium"
                          ...
                          <div class="flex-shrink-0 ml-1">
                            <Icon
                              ...
                              class="w-5 h-5 text-green-800 font-bold transition-transform duration-200 data-[state=open]:rotate-180"
                            />
                          </div>
                        </div>
                      </Accordion.Trigger>
                        ...
                        <div class="px-3 pb-2 space-y-1">
                          {#if item.items && item.items.length > 0}
                            {#each item.items as appInfo}
                              <a
                                href={getFileUrl(
                                  `fileType=2&appType=${parseInt(item.type)}&status=${appInfo.status}&titleType=custom`,
                                )}
    ...
    {#if showOnlyStatistics}
      <Accordion.Root class="space-y-4">
        ...
        <Accordion.Item
          value="trademarks"
          class="border border-slate-200/60 rounded-xl bg-gradient-to-r from-white via-slate-50/50 to-white hover:shadow-lg hover:shadow-green-500/10 transition-all duration-300 hover:scale-[1.01] hover:border-green-300/60"
        >
          <Accordion.Trigger
            class="hover:bg-transparent data-[state=open]:bg-transparent [&>div]:no-underline"
          >
              ...
              <div class="flex items-center space-x-3 flex-1">
                ...
                <div class="flex-1 text-left">
                  <div class="flex items-center gap-2 mb-0.5">
                    ...
                    <span
                      class="bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full text-xs font-semibold"
                      >{getType(FileTypes.Patent).length}</span
                    >
                  </div>
                  <p class="text-xs text-slate-500">
                    Protected inventions and innovations
              ...
              <div class="flex items-center space-x-3 flex-1">
                ...
                <div class="flex-1 text-left">
                  <div class="flex items-center gap-2 mb-0.5">
                    ...
                    <span
                      class="bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full text-xs font-semibold"
                      >{getType(FileTypes.Design).length}</span
                    >
                  </div>
                  <p class="text-xs text-slate-500">
                    Safeguarded creative designs
                  </p>
                </div>
              </div>
              <div class="flex-shrink-0 ml-2">
...
<style>
  ...
  :global(
    [data-accordion-trigger]
      svg:not(.w-6):not([class*="text-base"]):not([class*="w-5"])
  ) {
    display: none !important;
  }
