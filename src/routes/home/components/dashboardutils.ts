...
export function DataMapper(datatype: FileStatsData, data: FileStatsType[], isStaff: boolean) {
	...
	switch (datatype) {
		...
		case FileStatsData.newPatent:
			return (
				data.find(
					(x) => x.fileType === 0 && x.type === 0 && !listNotShow.includes(x.status as number)
				)?.count ?? 0
			);
		case FileStatsData.newDesignTotal: {
			...
			val = data
				.filter(
					(x) => x.fileType === 1 && x.type === 0 && !listNotShow.includes(x.status as number)
				)
				.map((x) => x.count)
				?.reduce((x, y) => x + y, 0);
			return val;
		...
		case FileStatsData.newPatentRenew:
			return (
				data.find(
					(x) => x.fileType === 0 && x.type === 1 && !listNotShow.includes(x.status as number)
				)?.count ?? 0
			);
		case FileStatsData.newDesignRenew:
			return (
				data.find(
					(x) => x.fileType === 1 && x.type === 1 && !listNotShow.includes(x.status as number)
				)?.count ?? 0
			);
		...
		case FileStatsData.newDesignUpdatePending: {
			const _filtered = data.filter(
				(x) => x.fileType === 1 && x.type === 2 && !listNotShow.includes(x.status as number)
			);
			const _filteredValues = _filtered.reduce(
				//new app: awaiting search: 20
				(acc, curr) => {
		...
		case FileStatsData.newPatentUpdatePending: {
			const _filtered = data.filter(
				(x) => x.fileType === 0 && x.type === 2 && !listNotShow.includes(x.status as number)
			);
			const _filteredValues = _filtered.reduce(
				//new app: awaiting search: 20
				(acc, curr) => {
