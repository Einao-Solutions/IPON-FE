<script lang="ts">
	...
	async function fetchData() {
		// guard: require year, quarter and type
	...
	// handlers moved here to avoid inline TS annotations in template
	function handleYearChange(selectin: any) {
		selectedYear = Number(selectin?.value);
	}

	function handleQuarterChange(selectin: any) {
		selectedQuarter = Number(selectin?.value);
	}

	function handleTypeChange(selectin: any) {
		selectedType = selectin?.value as string;
	}
</script>
...
<Dialog.Root bind:open={showRangePicker}>
	<Dialog.Content>
		...
		<div class="flex flex-col justify-between space-y-5 items-center">
			<Select.Root portal={null} onSelectedChange={handleYearChange}>
			...
			<Select.Root portal={null} onSelectedChange={handleQuarterChange} >
				<Select.Trigger class="w-[180px]">
			...
			<Select.Root portal={null} onSelectedChange={handleTypeChange} >
				<Select.Trigger class="w-[180px]">
			...
			<Button on:click={fetchData} class="w-[180px]" disabled={isFetching}>
