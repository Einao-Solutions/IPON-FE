<script lang="ts">
    ...
    try {
      ...
      if (searchParams) {
        ...
        const response = await fetch(
          `${baseURL}/api/migration/GetMarkInfo?regNumber=${encodeURIComponent(searchParams.query.trim())}`,
        );
        ...
        if (response.ok) {
          ...
          if (!responseText.trim()) {
            ...
            try {
              ...
              const files = Array.isArray(fileRecords)
                ? fileRecords
                : [fileRecords];
              results = files.map((file) => ({
                ...
                title:
                  ...
                  "",
                class:
                  file.class ?? file.tradeMarkClass ?? file.designClass ?? null,
  ...
  async function getPaymentDetails(paymentId: string) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        let markData = JSON.parse(
          sessionStorage.getItem("markInfo") || "[]",
        ) as SearchResult[];
  ...
  function validateCorrespondenceDetails(): boolean {
    ...
    const missingFields = required.filter(
      (field) =>
        !correspondenceDetails[field as keyof CorrespondenceDetails]?.trim(),
    );

    if (missingFields.length > 0) {
      toast.error(
        `Please fill in all correspondence details: ${missingFields.join(", ")}`,
        {
          position: "top-right",
        },
      );
  ...
  async function submitClaim() {
    ...
    try {
      ...
      formData.append(
        ...
        correspondenceDetails.nationality,
      );
...
<div class="space-y-4 m-4 p-2">
  ...
  {:else if error}
    <div
      class="bg-red-50 border border-red-200 p-6 rounded-md flex flex-col items-center text-center gap-3"
    >
      <Icon
...
<Dialog.Root bind:open={showPaymentDialog}>
  <Dialog.Content class="sm:max-w-md">
    <Dialog.Header>
      ...
      <Dialog.Description>
        Enter your payment ID to claim the file: <strong
          >{selectedResult?.fileNumber}</strong
        >
    ...
    <div class="space-y-4 py-4">
      ...
      >
        Please ensure to provide the RRR or Payment ID that is NOT an "IPON"
        number.
      </p>
    </div>
...
<Dialog.Root bind:open={showClaimDialog}>
  <Dialog.Content class="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
    <Dialog.Header>
      ...
      <Dialog.Description>
        Confirm your claim for file: <strong
          >{selectedResult?.fileNumber}</strong
        >
        - {selectedResult?.title}
      </Dialog.Description>
    ...
    <div class="space-y-6 py-4">
      ...
      <div class="bg-blue-50 p-4 rounded-md space-y-4">
        ...
        <p class="text-xs text-blue-700">
          Please provide contact information for correspondence regarding this
          file claim.
        </p>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="space-y-2">
            <Label for="correspondence-name"
      ...
      <div class="space-y-3">
        <Label for="attachments"
          >Supporting Documents <span class="text-red-500">*</span></Label
        >
        <div class="border-2 border-dashed border-gray-300 rounded-md p-4">
          ...
          <p class="text-xs text-gray-500 mt-2">
            Upload supporting documents (PDF, DOC, DOCX, JPG, PNG). Max 10MB per
            file.
          </p>
          <p class="text-sm text-gray-700 font-medium mt-3 mb-1">
            Acceptable Documents:
          </p>
          <ul
            class="list-disc list-inside text-sm text-gray-700 space-y-1 ml-1"
          >
      ...
      <div class="bg-yellow-50 p-4 rounded-md">
        <p class="text-sm text-yellow-800">
          By clicking "Submit Claim", you confirm that all the information
          provided is accurate and you wish to proceed with claiming this file.
          All future correspondence regarding this claim will be sent to the
          provided correspondence details.
        </p>
      </div>
    </div>
