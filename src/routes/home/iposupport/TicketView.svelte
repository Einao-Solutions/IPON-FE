﻿<script lang="ts">
  ...
  let selectedEscalationTarget: TicketCategory =
  ...
  $: isTrademarkSupport =
    $loggedInUser?.userRoles?.includes(UserRoles.TrademarkSupport) ?? false;
  $: isPatentDesignSupport =
    $loggedInUser?.userRoles?.includes(UserRoles.PatentDesignSupport) ?? false;
  $: isTechSupport =
    $loggedInUser?.userRoles?.includes(UserRoles.Tech) ?? false;
  $: isRegistrySupport = isTrademarkSupport || isPatentDesignSupport;

  $: escalationTargets = isTechSupport
    ? [
        {
          value: TicketCategory.TrademarkRegistry,
          label: "Trademark Registry Support",
        },
        {
          value: TicketCategory.PatentDesignRegistry,
          label: "Patent & Design Registry Support",
        },
      ]
    : isRegistrySupport
  ...
  function getMessageSenderName(message: {
    ...
    if (message.senderId === data?.creatorId && data?.creatorName?.trim())
  ...
  async function confirmAndEscalateTicket() {
    ...
    if (
      !window.confirm(
        `Confirm escalation to: ${escalationTargets.find((t) => t.value === selectedEscalationTarget)?.label ?? "the selected team"}?`,
      )
    ) {
      return;
    }
    ...
    isEscalating = true;
    const target = escalationTargets.find(
      (t) => t.value === selectedEscalationTarget,
    );
...
<AlertDialog.Root bind:open={showEscalateDialog}>
  <AlertDialog.Content>
    ...
    {#if escalationTargets.length > 1}
      <div class="flex flex-col gap-2 py-2">
        {#each escalationTargets as target}
          <label
            class="flex items-center gap-3 cursor-pointer p-3 rounded-lg hover:bg-slate-50 border-2 transition-colors {selectedEscalationTarget ===
            target.value
    ...
    {/if}
    <AlertDialog.Footer>
      <AlertDialog.Cancel on:click={() => (showEscalateDialog = false)}
        >Cancel</AlertDialog.Cancel
      >
      <Button
        disabled={isEscalating}
        on:click={confirmAndEscalateTicket}
        class="bg-red-600 hover:bg-red-700 text-white"
      >
        {#if isEscalating}<Icon
...
{#if open}
  ...
  >
    ...
    >
      ...
      <div
        class="flex items-center justify-between px-3 py-3 bg-white border-b flex-shrink-0"
      >
        <div class="flex items-center gap-3 min-w-0">
          ...
          <div
            class="flex-shrink-0 w-2 h-2 rounded-full {statusColor(
              data?.status ?? 2,
            )}"
          ></div>
          ...
          <h2 class="font-semibold text-slate-800 text-sm truncate">
            {data?.title ?? "—"}
          </h2>
        </div>
      ...
      <div class="flex flex-1 min-h-0 overflow-hidden">
        <!-- LEFT: meta panel -->
        <aside
          class="w-64 flex-shrink-0 border-r bg-white overflow-y-auto flex flex-col"
        >
          <!-- Creator -->
          <div class="px-4 pt-5 pb-4 border-b">
            <div class="flex items-center gap-3">
              <div
                class="w-9 h-9 rounded-full bg-gradient-to-br from-green-500 to-green-800 flex items-center justify-center text-white text-xs font-bold flex-shrink-0"
              >
          ...
          <div class="px-4 py-4 space-y-4 flex-1">
            <div>
              <p
                class="text-[10px] uppercase tracking-wide text-slate-400 mb-1"
              >
                Created
              </p>
              <p class="text-xs text-slate-700 font-medium">
                {data?.created ? mapDateToString(data.created) : "—"}
              </p>
            </div>
            ...
            {#if ticketCategory !== null}
              <div>
                ...
                <p class="text-xs text-slate-700 font-medium">
                  {mapCategoryToString(ticketCategory)}
                </p>
              </div>
            {/if}
            ...
            {#if data?.fileNumber}
              <div>
                ...
                <p class="text-xs text-slate-700 font-medium break-words">
                  {data.fileNumber}
                </p>
              </div>
            {/if}
            ...
            <div>
              ...
              <div class="flex items-center gap-1.5">
                <div
                  class="w-1.5 h-1.5 rounded-full {statusColor(
                    data?.status ?? 2,
                  )}"
                ></div>
                <p class="text-xs text-slate-700 font-medium">
                  {mapTicketStateToString(data?.status ?? 2)}
                </p>
              </div>
            </div>
              ...
              <div>
                ...
                <div class="flex items-center gap-1.5">
                  ...
                  <p class="text-xs text-red-600 font-medium">
                    Escalated{ticketEscalatedFromCategory !== null
                      ? ` from ${mapCategoryToString(ticketEscalatedFromCategory)}`
                      : ""}
                  </p>
                </div>
              </div>
            ...
            {#if data?.affectedFiles && data.affectedFiles.length > 0}
              <div>
                <p
                  class="text-[10px] uppercase tracking-wide text-slate-400 mb-2"
                >
                  Affected Files
                </p>
          ...
          <!-- Timeline mini -->
          <div
            class="px-4 py-4 border-t flex-shrink-0 overflow-y-auto max-h-52"
          >
            <p class="text-[10px] uppercase tracking-wide text-slate-400 mb-3">
              Timeline
            </p>
            <div class="relative pl-4">
              <div
                class="absolute left-1.5 top-0 bottom-0 w-px bg-slate-200"
              ></div>

              <div class="relative mb-3">
                <div
                  class="absolute -left-[13px] top-0.5 w-2 h-2 rounded-full bg-green-600 ring-2 ring-white"
                ></div>
                <p class="text-[10px] font-medium text-slate-600">Created</p>
                <p class="text-[10px] text-slate-400">
                  {data?.created ? relativeTime(data.created) : "—"}
                </p>
              </div>
              ...
              {#if data?.correspondences && data.correspondences.length > 1}
                <div class="relative mb-3">
                  <div
                    class="absolute -left-[13px] top-0.5 w-2 h-2 rounded-full bg-blue-500 ring-2 ring-white"
                  ></div>
                  <p class="text-[10px] font-medium text-slate-600">
                    First Reply
                  </p>
                  <p class="text-[10px] text-slate-400">
                    {relativeTime(data.correspondences[1]?.dateAdded ?? "")}
                  </p>
                </div>
              {/if}
              ...
              {#if ticketIsEscalated}
                <div class="relative mb-3">
                  <div
                    class="absolute -left-[13px] top-0.5 w-2 h-2 rounded-full bg-red-500 ring-2 ring-white"
                  ></div>
                  <p class="text-[10px] font-medium text-red-600">Escalated</p>
                  <p class="text-[10px] text-slate-400">
                    {dataExt?.escalatedAt
                      ? relativeTime(dataExt.escalatedAt)
                      : ""}
                  </p>
                </div>
              {/if}
              ...
              {#if data?.status === TicketStates.closed}
                <div class="relative">
                  <div
                    class="absolute -left-[13px] top-0.5 w-2 h-2 rounded-full bg-slate-500 ring-2 ring-white"
                  ></div>
                  <p class="text-[10px] font-medium text-slate-600">Closed</p>
                  <p class="text-[10px] text-slate-400">
                    {data?.resolution ? relativeTime(data.resolution.date) : ""}
                  </p>
                </div>
              {/if}
            </div>
              ...
              {#each data.correspondences as message, i}
                ...
                {#if isSystem}
                  ...
                  <div class="flex items-center justify-center gap-2 min-w-0">
                    ...
                    <div
                      class="flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-100 border border-red-200 max-w-[75%] min-w-0"
                    >
                      <Icon
                        icon="mdi:arrow-up-circle"
                        width="0.75rem"
                      ...
                      />
                      <span
                        class="text-[10px] font-medium text-red-700 break-words"
                        style="white-space:break-spaces;overflow-wrap:anywhere;tab-size:4;"
                ...
                {:else}
                  <div
                    class="flex {isCreator
                      ? 'flex-row-reverse'
                      : 'flex-row'} items-end gap-2 min-w-0"
                  >
                    <!-- Avatar -->
                    <div
                      class="w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center text-white text-[10px] font-bold {isCreator
                    ...
                    <!-- Bubble -->
                    <div
                      class="max-w-[78%] min-w-0 flex flex-col {isCreator
                        ? 'items-end'
                        : 'items-start'}"
                    >
                      <div
                        class="flex items-center gap-2 mb-1 px-1 text-[10px] text-slate-400"
                      >
                        <span class="font-medium text-slate-600"
                          >{getMessageSenderName(message)}</span
                        >
                        <span>·</span>
                        <span>{relativeTime(message.dateAdded)}</span>
                      </div>
                      <div
                        class="px-4 py-3 rounded-2xl text-sm leading-relaxed break-words shadow-sm min-w-0
{isCreator
                      ...
                      >
                        ...
                        {#if message.attachment}
                          <a
                            ...
                            href={`${baseURL}${message.attachment}`}
                            class="mt-2 inline-flex items-center gap-1 text-xs font-medium {isCreator
          ...
          <div class="border-t bg-white flex-shrink-0">
            {#if data?.status !== TicketStates.closed}
              <div class="p-3 space-y-2">
                <Textarea
                  class="min-h-16 max-h-40 resize-none text-sm"
                ...
                <div class="flex items-center gap-2">
                  ...
                  {#if selectedFileName}
                    <div
                      class="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-green-100 border border-green-300 text-xs text-green-800 min-w-0 max-w-[180px]"
                    >
                      <Icon
                        icon="mdi:paperclip"
                        width="0.8rem"
                      ...
                      <button
                        type="button"
                        on:click={clearFile}
                        class="text-green-600 hover:text-red-500 flex-shrink-0"
                        aria-label="Remove"
                      >
                        <Icon
                          icon="mdi:close"
                  ...
                  {:else}
                    <label
                      class="flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-slate-200 text-xs text-slate-500 cursor-pointer hover:border-green-400 hover:text-green-700 transition-colors"
                    >
                      <Icon
                        icon="mdi:paperclip"
                        width="0.85rem"
                  ...
                  <Button
                    size="sm"
                    class="h-8 px-4"
                    disabled={isUploading || (!newText.trim() && !selectedFile)}
            ...
            {:else}
              <div
                class="px-4 py-3 flex items-center gap-2 text-sm text-slate-500 bg-slate-50"
              >
                <Icon
                ...
                />
                <span>
                  Ticket closed by <strong
                    >{data.resolution?.staffName ?? "—"}</strong
                  >
                  {#if data.resolution}
                    · {mapDateToString(data.resolution.date)}{/if}
                </span>
              </div>
            {/if}
          </div>
