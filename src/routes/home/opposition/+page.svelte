<script lang="ts">
  ...
  async function loadOppositionStats() {
    ...
    if (!stats.awaitingOfficeProcess) {
      const res = await fetch(
        `${baseURL}/api/opposition/loadSummary?quantity=1&skip=0&type=36`,
        { headers },
      );
      if (res.ok) {
  ...
  async function loadWCounts() {
    try {
      ...
      const [r1, r2, r3] = await Promise.all([
        fetch(
          `${baseURL}/api/opposition/loadSummary?quantity=1&skip=0&type=${ApplicationStatuses.AwaitingPayment}`,
          { headers },
        ),
        fetch(
          `${baseURL}/api/opposition/loadSummary?quantity=1&skip=0&type=${ApplicationStatuses.RequestWithdrawal}`,
          { headers },
        ),
        fetch(
          `${baseURL}/api/opposition/loadSummary?quantity=1&skip=0&type=${ApplicationStatuses.Withdrawn}`,
          { headers },
        ),
      ]);
  ...
  async function confirmTreat() {
    ...
    try {
      const officerRole =
        $loggedInUser?.userRoles?.find((r: number) =>
          [
            UserRoles.TrademarkOpposition,
            UserRoles.Tech,
            UserRoles.SuperAdmin,
      ...
      if (res.ok && data.success !== false) {
        ...
        toast.error(
          data.message ?? "Failed to process the withdrawal request.",
          { position: "top-right" },
        );
        showConfirm = false;
        showTreatModal = true;
...
<div class="flex flex-col space-y-4">
  <div class="flex items-center gap-2 border-b border-slate-200 pb-1">
    <button
      class="px-5 py-2 rounded-t-lg text-sm font-semibold transition-colors {mainView ===
      'applications'
  ...
  {#if mainView === "withdrawal"}
    <div class="flex flex-col space-y-5">
      <div class="flex flex-wrap gap-3">
        <button
          class="group relative overflow-hidden {withdrawalTab ===
          'awaiting-payment'
            ? 'bg-red-600'
            : 'bg-red-500 hover:bg-red-600'} text-white border-0 shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:scale-[1.02] px-6 py-3 rounded-xl"
        ...
        <button
          class="group relative overflow-hidden {withdrawalTab === 'requested'
            ? 'bg-amber-600'
            : 'bg-amber-500 hover:bg-amber-600'} text-white border-0 shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:scale-[1.02] px-6 py-3 rounded-xl"
        ...
        <button
          class="group relative overflow-hidden {withdrawalTab === 'withdrawn'
            ? 'bg-green-700'
            : 'bg-green-600 hover:bg-green-700'} text-white border-0 shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:scale-[1.02] px-6 py-3 rounded-xl"
                ...
                {#each wItems as row, i}
                  <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="px-4 py-3 text-slate-700 whitespace-nowrap"
                      >{row.date ? mapDateToString(row.date) : "-"}</td
                    >
    ...
    {:else if oppositionDetail}
      ...
      <div class="p-6 space-y-6">
          ...
          <div class="grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <p class="font-medium text-slate-800">
                {oppositionDetail.fileId ?? oppositionDetail.fileNumber ?? "-"}
              </p>
            </div>
            <div>
              <p class="text-slate-500">Title</p>
              <p class="font-medium text-slate-800">
                {oppositionDetail.fileTitle ?? oppositionDetail.title ?? "-"}
              </p>
            </div>
            <div>
              <p class="text-slate-500">Opposer Name</p>
              <p class="font-medium text-slate-800">
                {oppositionDetail.name ?? oppositionDetail.opposerName ?? "-"}
              </p>
            </div>
            <div>
              <p class="text-slate-500">Opposer Email</p>
              <p class="font-medium text-slate-800">
                {oppositionDetail.email ?? "-"}
              </p>
            </div>
            <div>
              <p class="text-slate-500">Date Filed</p>
              <p class="font-medium text-slate-800">
                {oppositionDetail.date
                  ? mapDateToString(oppositionDetail.date)
                  : "-"}
              </p>
            </div>
            <div>
              ...
              <p class="font-medium text-slate-800">
                {oppositionDetail.paymentId ?? selectedItem?.paymentId ?? "-"}
              </p>
            </div>
          </div>
        </div>
        ...
        {#if oppositionDetail.withdrawalReason}
          ...
          >
            <h3
              class="font-semibold text-amber-700 text-sm uppercase tracking-wide flex items-center gap-1"
            >
              <Icon icon="mdi:text-box-outline" class="w-4 h-4" />
              Reason for Withdrawal
            </h3>
            <p class="text-sm text-slate-700">
              {oppositionDetail.withdrawalReason}
            </p>
          </div>
        {/if}
        ...
        {#if oppositionDetail.withdrawalDocument || oppositionDetail.attachments?.length}
          ...
          >
            <h3
              class="font-semibold text-slate-700 text-sm uppercase tracking-wide flex items-center gap-1"
            >
            ...
            {#if oppositionDetail.withdrawalDocument}
              <a
                href={oppositionDetail.withdrawalDocument}
                target="_blank"
          ...
          >
            ...
            <div class="space-y-2">
              {#each oppositionDetail.history ?? oppositionDetail.applicationHistory ?? [] as h}
                <div
                  class="flex items-start gap-3 text-sm border-l-2 border-slate-200 pl-3 py-1"
                >
                  <div>
                    <p class="font-medium text-slate-700">
                      {h.action ?? h.status ?? h.description ?? "-"}
                    </p>
                    <p class="text-slate-400 text-xs">
                      {h.date ? mapDateToString(h.date) : ""}
                    </p>
                  </div>
                </div>
              {/each}
    ...
    <div class="space-y-4 py-2">
      <div class="space-y-1.5">
        ...
        <Textarea
          ...
          bind:value={treatReason}
          class="min-h-28"
        />
      </div>
    </div>
    <Dialog.Footer class="flex gap-2 pt-2">
      <Button variant="outline" on:click={() => (showTreatModal = false)}
        >Cancel</Button
      >
...
<Dialog.Root bind:open={showConfirm}>
  <Dialog.Content class="sm:max-w-sm">
    <Dialog.Header>
      ...
      <Dialog.Description>
        {#if treatAction === "approve"}
          Are you sure you want to approve this withdrawal? The opposition
          status will be set to Withdrawn and the file status will be updated to
          Awaiting Certification.
        {:else}
          Are you sure you want to refuse this withdrawal? No status changes
          will be made.
        {/if}
      </Dialog.Description>
    </Dialog.Header>
