<script lang="ts">
	...
	async function handleSubmit() {
		...
		try {
			...
			form.append('document', userData.document as File);

			

			const res = await fetch(`${baseURL}/api/files/AddRegisteredUsers`, {
				method: 'POST',
				body: form
			});
			if (res.ok) {
	...
	async function handlePayment() {
		if (cost && paymentId) {
			await goto(`/payment/?type=registeredusers&rrr=${paymentId}&amount=${cost}`);
		}
	}
	function goBack() {
		window.history.back();
	...
	<div class="w-full mx-auto">
		...
		<div class="flex items-center">
			...
			<div class="flex-1 flex flex-col items-center justify-center">
				<h1 class="text-xl font-bold">Registered Users Application [Form 47]</h1>
				<p class="font-light">Fill in the details for a registered user of your trademark</p>
			</div>
		</div>
		...
		<div class="px-6 py-6">
			...
			<div class="mb-6 border border-gray-300 rounded-md overflow-hidden">
				<div class="bg-gray-300 px-4 py-2 font-medium text-black">REGISTERED USER FORM</div>
				{#if isLoading}
			...
			<div class="mb-6 border border-gray-300 rounded-md overflow-hidden">
				<div class="bg-gray-300 px-4 py-2 font-medium text-black">REGISTERED USER INFORMATION</div>
				<div class="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
					<div>
						<!-- svelte-ignore a11y-label-has-associated-control -->
						<label class="block text-sm font-medium text-gray-700 mb-1">
