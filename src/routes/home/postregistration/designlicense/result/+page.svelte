<script lang="ts">
	...
	async function submitToBackend() {
		try {
			const payloadData = sessionStorage.getItem('designLicensePayload');
			if (!payloadData) {
			...
			const response = await fetch(`${baseURL}/api/files/DesignLicenseApplication`, {
				method: 'POST',
			...
			sessionStorage.removeItem('designLicensePayload');
			sessionStorage.removeItem('designLicenseFormData');

			localStorage.setItem('designLicenseSubmission', JSON.stringify({
			...
			toast.success('Design license application submitted successfully!');
		} catch (err) {
...
<svelte:head>
	<title>Design License Application - Submission Result</title>
</svelte:head>
...
{#if isLoading}
	<main class="flex justify-center items-center min-h-screen bg-gray-50">
		<div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
			...
			<p class="text-gray-600">Submitting your design license application...</p>
		</div>
	</main>
{:else if submissionError}
	<main class="flex justify-center items-center min-h-screen bg-gray-50">
		<div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
			...
			<button class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-6 rounded-md transition-colors duration-200 mr-3" on:click={() => goto('/home/postregistration/designlicense')}>Try Again</button>
		...
		<div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
			...
			{#if subMessageVisible}
				<p class="text-base font-medium text-gray-600 mb-8 max-w-xs mx-auto" in:fly={{ y: 15, duration: 500 }}>
					YOUR DESIGN LICENSE APPLICATION HAS BEEN RECEIVED AND IS RECEIVING DUE ATTENTION
				</p>
