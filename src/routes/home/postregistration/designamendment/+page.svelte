					...
					<div class="mb-6 border border-gray-300 rounded-md overflow-hidden">
						<div class="bg-gray-300 px-4 py-2 font-medium text-black flex items-center justify-between">
							...
							{#if existingAttachments.length + newAttachmentFiles.length > 0}
								<span class="text-sm bg-green-800 text-white px-3 py-1 rounded-full">{existingAttachments.length + newAttachmentFiles.length} file{(existingAttachments.length + newAttachmentFiles.length) !== 1 ? 's' : ''}</span>
							{/if}
						</div>
						<div class="p-4">
									...
									<div class="border rounded-lg p-3 bg-gray-50 hover:bg-gray-100 transition-colors">
										<div class="flex items-center gap-3">
											...
											<div class="flex-shrink-0 flex items-center gap-2">
												<a href={attachment.url} target="_blank" rel="noopener noreferrer" on:click={(e) => e.stopPropagation()} class="bg-green-800 hover:bg-green-700 text-white px-3 py-2 rounded-lg text-sm font-medium transition-colors inline-flex items-center gap-2 shadow-sm whitespace-nowrap"><Icon icon="mdi:file-eye" width="1.2em" height="1.2em" /><span>View</span></a>
