<script lang="ts">
	...
	async function submitToBackend() {
		try {
			const payloadData = sessionStorage.getItem('patentMergerPayload');
			if (!payloadData) {
			...
			const response = await fetch(`${baseURL}/api/files/PatentMergerApplication`, {
				method: 'POST',
				headers: {
			...
			sessionStorage.removeItem('patentMergerPayload');
			sessionStorage.removeItem('patentMergerFormData');
			
			// Store success data for potential future reference
			localStorage.setItem('patentMergerSubmission', JSON.stringify({
			...
			toast.success('Patent merger application submitted successfully!');
...
<svelte:head>
	<title>Patent Merger Application - Submission Result</title>
</svelte:head>
...
{#if isLoading}
	<main class="flex justify-center items-center min-h-screen bg-gray-50">
		<div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
			...
			<p class="text-gray-600">Submitting your patent merger application...</p>
		</div>
	</main>
	...
	<main class="flex justify-center items-center min-h-screen bg-gray-50">
		<div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
			...
			<button
				...
				on:click={() => goto('/home/postregistration/patentmerger')}
			>
				Try Again
			</button>
			<button
	...
	<main class="flex justify-center items-center min-h-screen bg-gray-50">
		<div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
			...
			{#if subMessageVisible}
				...
				>
					YOUR PATENT MERGER APPLICATION HAS BEEN RECEIVED AND IS RECEIVING DUE ATTENTION
				</p>
