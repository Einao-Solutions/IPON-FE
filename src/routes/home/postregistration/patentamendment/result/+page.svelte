<script lang="ts">
	...
	async function submitToBackend() {
		try {
			const payloadData = sessionStorage.getItem('patentAmendmentPayload');
			if (!payloadData) {
			...
			const response = await fetch(`${baseURL}/api/files/PatentAmendmentApplication`, {
				method: 'POST',
				headers: {
			...
			sessionStorage.removeItem('patentAmendmentPayload');
			
			// Store success data for potential future reference
			localStorage.setItem('patentAmendmentSubmission', JSON.stringify({
			...
			toast.success('Patent amendment application submitted successfully!');
...
<svelte:head>
	<title>Patent Amendment - Submission Result</title>
</svelte:head>
...
{#if isLoading}
	<main class="flex justify-center items-center min-h-screen bg-gray-50">
		<div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
			...
			<p class="text-gray-600">Submitting your patent amendment application...</p>
		</div>
	</main>
	...
	<main class="flex justify-center items-center min-h-screen bg-gray-50">
		<div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
			...
			<button
				...
				on:click={() => goto('/home/postregistration/patentamendment')}
			>
				Try Again
			</button>
			<button
	...
	<main class="flex justify-center items-center min-h-screen bg-gray-50">
		<div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
			...
			{#if subMessageVisible}
				...
				>
					YOUR PATENT AMENDMENT APPLICATION HAS BEEN RECEIVED AND IS RECEIVING DUE ATTENTION
				</p>
