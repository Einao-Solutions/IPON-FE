<script lang="ts">
	...
	async function submitToBackend() {
		try {
			const payloadData = sessionStorage.getItem('patentMortgagePayload');
			if (!payloadData) {
			...
			const response = await fetch(`${baseURL}/api/files/PatentMortgageApplication`, {
				method: 'POST',
				headers: {
			...
			sessionStorage.removeItem('patentMortgagePayload');
			
			// Store success data for potential future reference
			localStorage.setItem('patentMortgageSubmission', JSON.stringify({
			...
			toast.success('Patent mortgage application submitted successfully!');
...
<svelte:head>
	<title>Patent Mortgage Application - Submission Result</title>
</svelte:head>
...
{#if isLoading}
	<main class="flex justify-center items-center min-h-screen bg-gray-50">
		<div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
			...
			<p class="text-gray-600">Submitting your patent mortgage application...</p>
		</div>
	</main>
	...
	<main class="flex justify-center items-center min-h-screen bg-gray-50">
		<div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
			...
			<button
				...
				on:click={() => goto('/home/postregistration/patentmortgage')}
			>
				Try Again
			</button>
			<button
	...
	<main class="flex justify-center items-center min-h-screen bg-gray-50">
		<div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
			...
			{#if subMessageVisible}
				...
				>
					YOUR PATENT MORTGAGE APPLICATION HAS BEEN RECEIVED AND IS RECEIVING DUE ATTENTION
				</p>
