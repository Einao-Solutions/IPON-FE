<script lang="ts">
	...
	async function submitToBackend() {
		try {
			const payloadData = sessionStorage.getItem('patentLicensePayload');
			if (!payloadData) {
			...
			const response = await fetch(`${baseURL}/api/files/PatentLicenseApplication`, {
				method: 'POST',
				headers: {
		...
		sessionStorage.removeItem('patentLicensePayload');
			// Store success data for potential future reference
			localStorage.setItem('patentLicenseSubmission', JSON.stringify({
			...
			toast.success('Patent license application submitted successfully!');
...
<svelte:head>
	<title>Patent License Application - Submission Result</title>
</svelte:head>
...
{#if isLoading}
	<main class="flex justify-center items-center min-h-screen bg-gray-50">
		<div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
			...
			<p class="text-gray-600">Submitting your patent license application...</p>
		</div>
	</main>
	...
	<main class="flex justify-center items-center min-h-screen bg-gray-50">
		<div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
			...
			<button
				...
				on:click={() => goto('/home/postregistration/patentlicense')}
			>
				Try Again
			</button>
			<button
	...
	<main class="flex justify-center items-center min-h-screen bg-gray-50">
		<div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-md text-center">
			...
			{#if subMessageVisible}
				...
				>
					YOUR PATENT LICENSE APPLICATION HAS BEEN RECEIVED AND IS RECEIVING DUE ATTENTION
				</p>
