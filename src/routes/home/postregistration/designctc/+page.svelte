<script lang="ts">
	...
	async function setData(fileNumber: string, fileType: string): Promise<void> {
		...
		try {
			...
			applicantName = data.applicantName; designTitle = data.fileTitle || data.titleOfDesign || '';
		...
		} catch (err) { error = 'Error fetching design CTC information.'; console.error(err); } finally { isLoading = false; }
	}
	...
	async function handleSubmit() {
		...
		try {
			...
			const rrrResponse = await fetch(`${baseURL}/api/files/GetDesignCTCCost?fileId=${fileId}&fileType=0&numberOfAttachments=${selectedAttachmentIndices.length}`);
...
<div class="min-h-screen py-4 px-4">
	<div class="w-full mx-auto">
		...
		<div class="px-6 py-6">
							...
							{#each attachments as attachment, index}
								<div class="border rounded-lg p-3 bg-gray-50 hover:bg-gray-100 transition-colors {selectedAttachmentIndices.includes(index) ? 'border-blue-500 bg-blue-50' : ''}">
									<div class="flex items-center gap-3">
										<input type="checkbox" id="attachment-{index}" checked={selectedAttachmentIndices.includes(index)} on:change={() => toggleAttachmentSelection(index)} class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
										<div class="flex-1 min-w-0">
											<label for="attachment-{index}" class="cursor-pointer">
												<div class="font-medium text-gray-800">{attachment.name || attachment.fileName || `Document ${index + 1}`}</div>
												<div class="text-xs text-gray-500 mt-1">Document {index + 1} of {attachments.length}</div>
											</label>
										</div>
										<div class="flex-shrink-0">
											{#if attachment.url}
												<a href={attachment.url} target="_blank" rel="noopener" class="bg-green-800 hover:bg-green-700 text-white px-3 py-2 rounded-lg text-sm font-medium transition-colors inline-flex items-center gap-2 shadow-sm whitespace-nowrap" on:click={(e) => e.stopPropagation()}><Icon icon="mdi:file-eye" width="1.2em" height="1.2em" /><span>View</span></a>
											{:else}<span class="text-gray-400 px-3 py-2 rounded-lg text-sm">No URL</span>{/if}
										</div>
									</div>
								</div>
							{/each}
			...
			<div class="mb-6 border border-gray-300 rounded-md overflow-hidden">
				...
				<div class="p-4">
					...
					{#if selectedAttachmentIndices.length === 0}
						<div class="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg"><p class="text-sm text-amber-800"><Icon icon="mdi:alert" class="inline" width="1.2em" height="1.2em" /> Please select at least one document to proceed</p></div>
					{/if}
				</div>
			</div>
			...
			<div class="flex justify-end">
				{#if baseCost !== null}
					<button on:click={handleSubmit} class="bg-green-800 text-white px-6 py-2 rounded-md hover:bg-green-700 transition-colors flex items-center disabled:bg-gray-400 disabled:cursor-not-allowed" disabled={isProcessing || selectedAttachmentIndices.length === 0}>
	...
	<Dialog.Root bind:open={showExistingApplicationModal}>
		<Dialog.Content class="w-11/12 max-w-md mx-auto">
			...
			<div class="py-4"><p class="text-gray-700">A design CTC application has already been submitted for this file.</p></div>
