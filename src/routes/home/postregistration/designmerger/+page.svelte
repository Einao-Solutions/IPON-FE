<script lang="ts">
	...
	async function setData(fileNumber: string, fileType: string): Promise<void> {
		...
		} catch (err) { error = 'Error fetching design merger cost.'; console.error(err); } finally { isLoading = false; }
	}
	...
	async function handleSubmit() {
		...
		try {
			...
			for (const file of formData.mergerDeeds) {
				...
				mergerDeedData.push({ fileName: file.name, contentType: file.type, data: base64Data, name: 'Deedofmerger' });
			}
			const supportingDocsData = [];
			for (const file of formData.supportingDocuments) {
				const base64Data = await convertToBase64(file);
				supportingDocsData.push({ fileName: file.name, contentType: file.type, data: base64Data, name: 'DesignMergerSupportingDocuments' });
			}
			const now = new Date();
			const payload = {
				...
				newMergerName: newMergerData.name, newMergerEmail: newMergerData.email, newMergerPhone: newMergerData.phone,
				newMergerAddress: newMergerData.address, newMergerNationality: newMergerData.nationality,
				newMergerCity: newMergerData.city, newMergerState: newMergerData.state
			};
			...
			sessionStorage.setItem('designMergerFormData', JSON.stringify({
				mergerDeeds: formData.mergerDeeds.map(f => f.name), supportingDocuments: formData.supportingDocuments.map(f => f.name),
				designTitle, applicantName, applicantEmail, fileId
			}));
			await handlePayment();
	...
	async function handlePayment() {
		if (cost && paymentId) { await goto(`/payment/?type=designmerger&rrr=${paymentId}&amount=${cost}&fileId=${fileId}`); }
		else { toast.error('Unable to proceed to payment. Invoice could not be generated. Please try again.'); }
	}
...
<div class="min-h-screen py-4 px-4">
	<div class="w-full mx-auto">
		...
		<div class="px-6 py-6">
			...
			<div class="mb-6 border border-gray-300 rounded-md overflow-hidden">
				...
				{#if isLoading}
					<div class="flex items-center justify-center p-12"><div class="flex flex-col items-center gap-2"><Icon icon="line-md:loading-loop" width="2rem" height="2rem" class="text-blue-600" /><span class="text-sm text-gray-500">Loading Current Design Holder Information...</span></div></div>
				{:else}
					<div class="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
			...
			<div class="mb-6 border border-gray-300 rounded-md overflow-hidden">
				...
				<div class="p-4 space-y-6">
					<div>
						...
						<div class="flex items-center mb-3"><input type="file" accept=".pdf,.doc,.docx" on:change={(e) => handleFileChange(e, 'mergerDeeds')} class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500" multiple /></div>
						{#if formData.mergerDeeds.length > 0}<div class="space-y-2"><p class="text-sm text-gray-600">Uploaded files:</p>{#each formData.mergerDeeds as file, index}<div class="flex items-center gap-2"><input class="flex-1 px-3 py-2 border border-gray-300 rounded-md bg-gray-100" value={file.name} readonly /><button type="button" class="flex items-center justify-center w-8 h-8 bg-blue-100 hover:bg-blue-200 rounded-md transition-colors" on:click={() => { const url = URL.createObjectURL(file); window.open(url, '_blank'); }} title="View file"><svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg></button><button type="button" class="px-3 py-1 text-xs bg-red-600 text-white rounded-md hover:bg-red-700 transition-colors" on:click={() => removeFile('mergerDeeds', index)}>Remove</button></div>{/each}</div>{/if}
	...
	<Dialog.Root bind:open={showExistingApplicationModal}>
		<Dialog.Content class="w-11/12 max-w-md mx-auto">
			...
			<div class="py-4"><p class="text-gray-700">A design merger application has already been submitted for this file. You cannot create multiple merger applications for the same design.</p></div>
