<script lang="ts">
  ...
  interface SearchResult {
    ...
    fileId: string | null;
    fileStatus: ApplicationStatuses | number;
    ...
    try {
      ...
      if (searchParams) {
        ...
        if (isPatent) {
          ...
          filteredResults = results.filter(
            (result) =>
              [ApplicationStatuses.AwaitingCertification].includes(
                result.fileStatus,
  ...
  function renewApplication(result: SearchResult) {
    ...
    if (isPatent) {
      ...
      if (currentStatus !== 0 && currentStatus !== 1) {
        toast.error("Patent file must be Active or Inactive.", {
          position: "top-right",
        });
        return;
      }
      ...
      if (["pct", "conventional"].includes(patentTypeStr)) {
        ...
        if (
          !Array.isArray(result.firstPriorityInfo) ||
          result.firstPriorityInfo.length === 0
        ) {
          priorityModalMessage =
    ...
    >
      ...
      >
        ...
        <Button
          ...
          on:click={closePriorityModal}>Cancel</Button
        >
      </div>
    </div>
  {/if}
