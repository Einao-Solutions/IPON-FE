<script lang="ts">
	...
	import { mapDateToStringNoDate, mapTypeToString } from '../../components/dashboardUtils';
	...
	async function updatePaymentId(application) {
		try {
			const newPaymentId = prompt('Enter new Payment ID:', application.paymentId || '');
		...
		<div class="flex-1 overflow-y-auto py-4">
			{#if loading}
				<div class="flex items-center justify-center py-12">
					<Icon icon="mdi:loading" width="2em" height="2em" class="animate-spin text-green-800" />
			...
			{:else if applications.length === 0}
				<div class="flex flex-col items-center justify-center py-12 text-gray-500">
					...
					<p class="text-sm">No applications are associated with file {fileNumber}</p>
				</div>
			{:else}
				<div class="space-y-4">
