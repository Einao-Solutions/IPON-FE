...
export const PatentSections = [
	{ name: 'basic', icon: 'material-symbols:info-outline', description: 'Patent Information' },
	{ name: 'inventors', icon: 'streamline:ai-science-spark', description: 'inventors' },
	...
	{ name: 'priority', description: 'Priority Info', icon: 'pepicons-print:letter' },
	{ name: 'attachments', description: 'Attachments', icon: 'teenyicons:attachment-outline' },
	{ name: 'verification', description: 'Verification', icon: 'ooui:check-all' }
];
export const DesignSections = [
	{ name: 'designBasic', icon: 'material-symbols:info-outline', description: 'Design information' },
	{ name: 'designCreators', icon: 'streamline:ai-science-spark', description: 'creators' },
	{ name: 'applicant', description: 'applicants', icon: 'mdi:people-outline' },
	{
		name: 'correspondence',
		description: 'correspondence',
...
export function getStatusColour(status: ApplicationStatuses) {
	switch (status) {
		...
		case ApplicationStatuses.AwaitingExaminer || ApplicationStatuses.AwaitingCounter:
			return '#9B870C';
...
export type UsersType = {
	...
	defaultCorrespondence: CorrespondenceType | undefined;
	roles: UserRoles[];
};
...
export type PerformanceData = {
	...
	treatedCount: { amount: number; user: string; fileType: number; before: number; after: number }[];
	endDate: string;
	patent: {
		newCreations: number;
		renewals: number;
...
export function hasValidCorrespondenceDetails(data?: CorrespondenceType) {
	...
	const validNigerianNumber = isValidPhoneNumber(data?.phone?.trim() ?? '', 'NG');
	const validAddress =
		data?.address !== '-' &&
